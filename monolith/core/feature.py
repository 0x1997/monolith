...
class FeatureSlot(object):
  ...
  def __init__(self,
               ...
               slot_id,
               has_bias=False,
               bias_optimizer=tf.compat.v1.tpu.experimental.FtrlParameters(learning_rate=0.01),
               bias_initializer=tf.zeros_initializer(),
               bias_learning_rate_fn=None,
               default_vec_optimizer=tf.compat.v1.tpu.experimental.AdagradParameters(learning_rate=0.01),
               default_vec_initializer=tf.random_uniform_initializer(minval=-0.001, maxval=0.001),
  ...
  def _add_feature_column(self, feature_column):
    ...
    if self._has_bias:
      for feature_column in self._feature_columns:
        feature_column._bias = feature_column.embedding_lookup(self._feature_slices[0])
...
class FeatureColumnV1(object):
  ...
  def embedding_lookup(self, feature_slice, init_minval_for_oov=None, init_maxval_for_oov=None,):
    return self.get_env()._embedding_lookup(self, feature_slice, init_minval_for_oov, init_maxval_for_oov)
...
class FeatureColumn3D(object):
  ...
  def embedding_lookup(self, feature_slice, max_seq_length, init_minval_for_oov=None, init_maxval_for_oov=None,):
    return self.get_env()._seq_embedding_lookup(self, feature_slice, self._max_seq_length, init_minval_for_oov, init_maxval_for_oov)
...
class Env(object):
  ...
  def _embedding_lookup(self, feature_column, feature_slice, init_minval_for_oov=None, init_maxval_for_oov=None):
    assert feature_column._feature_slot.slot_id() == feature_slice._feature_slot.slot_id()
    ...
    if self._tpu_features:
      ...
      if self._QR_multi_hashing and self._vocab_size_dict[
        ...
        logging.vlog(2,
                     "__embedding_lookup of QR hashing for slot {}.".format(slot_id))
        # taking quotient feature and remainder feature
        keyR = "{}_{}_0".format(feature_column.fc_name, feature_slice.slice_index)
        keyQ = "{}_{}_1".format(feature_column.fc_name, feature_slice.slice_index)
      ...
      if feature_slice not in feature_column._feature_slice_to_tf_placeholder:
        feature_column._feature_slice_to_tf_placeholder[feature_slice] = tf.compat.v1.placeholder(tf.float32, [None, feature_slice.dim])

      return feature_column._feature_slice_to_tf_placeholder[feature_slice]

  def _seq_embedding_lookup(self, feature_column, feature_slice, max_seq_length, init_minval_for_oov=None, init_maxval_for_oov=None):
    assert feature_column._feature_slot.slot_id() == feature_slice._feature_slot.slot_id()
    ...
    if self._tpu_features:
      ...
      logging.vlog(2, "__embedding_lookup with no features exist.")
      if feature_slice not in feature_column._feature_slice_to_tf_placeholder:
        feature_column._feature_slice_to_tf_placeholder[feature_slice] = tf.compat.v1.placeholder(tf.float32, [None, max_seq_length, feature_slice.dim])
  ...
  def _size_tensor_lookup(self, feature_column):
    if self._tpu_features:
      ...
      boolean_mask = tf.less(
        ...
        tf.expand_dims(row_lengths, 1)
      ) # [B, max_seq_length] Tensor
    ...
    for feature_column in feature_slot.feature_columns:
      ...
      for merged_feature_slice in feature_column._merged_feature_slice_to_tf_placeholder:
        if merged_feature_slice.slice_index == 0 and feature_slot.has_bias():
          ...
          assert merged_feature_slice.dim == 1, "Bias in {} must have dim equal to 1, but actual dim is {}.".format(feature_column.fc_name, merged_feature_slice.dim)
        else:
          merged_feature_name = "{}_{}".format(feature_column.fc_name, merged_feature_slice.slice_index)
      ...
      if merged_embedding is not None:
        ...
        dim_splits = [feature_slice.dim for feature_slice in feature_slot.feature_slices]
        if feature_slot.has_bias():
          dim_splits = [feature_slice.dim for feature_slice in feature_slot.feature_slices][1:]
      ...
      for feature_slice in feature_column._feature_slice_to_tf_placeholder:
        if feature_slice.slice_index == 0 and feature_slot.has_bias():
          # For bias, keep it as bias will not be merged. Nothing to do.
          assert feature_slice.dim == 1, "Bias in {} must have dim equal to 1, but actual dim is {}.".format(feature_column.fc_name, feature_slice.dim)
        else:
          if merged_embedding is not None:
            if feature_slot.has_bias():
            ...
            self._tpu_features["{}_{}".format(feature_column.fc_name, feature_slice.slice_index)] = split
    ...
    for slot_id, feature_slot in self._slot_id_to_feature_slot.items():
      ...
      for feature_slice in feature_slot.feature_slices:
        ...
        if feature_slot.has_bias() and feature_slice.slice_index == 0:
          assert feature_slice.dim == 1, "Bias in {} must have dim equal to 1, but actual dim is {}.".format(slot_id, feature_slice.dim)
          feature_slot._merged_feature_slices.append(feature_slice)
          for feature_column in feature_slot.feature_columns:
            feature_column._merged_feature_slice_to_tf_placeholder[feature_slice] = feature_column._feature_slice_to_tf_placeholder[feature_slice]
        else:
      ...
      if merged_vector_dim > 0:
        ...
        for feature_column in feature_slot.feature_columns:
          feature_column._merged_feature_slice_to_tf_placeholder[merged_feature_slice] = tf.compat.v1.placeholder(tf.float32, [None, merged_feature_slice.dim])
