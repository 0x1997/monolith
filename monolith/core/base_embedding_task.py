...
class BaseEmbeddingTask(base_task.BaseTask):
  ...
  def params(cls):
    ...
    p.define('vocab_size_offset', None,
  ...
  def _post_process_example(self, example):
    ...
    for slot_id, feature_slot in list(self._env.slot_id_to_feature_slot.items()):
        ...
        for feature_column in feature_slot.feature_columns:
          ...
          if self.p.qr_multi_hashing and vocab_size > self.p.qr_hashing_threshold:
            ...
            for feature_slice in feature_column.feature_slice_to_tf_placeholder:
              example["{}_{}_0".format(feature_column.fc_name, feature_slice.slice_index)] = tf.SparseTensor(
                  indices=embedding_tensor.indices,
                  values=tf.math.floormod(embedding_tensor.values, R_vocab_size),
                  dense_shape=embedding_tensor.dense_shape)
              example["{}_{}_1".format(feature_column.fc_name, feature_slice.slice_index)] = tf.SparseTensor(
                  indices=embedding_tensor.indices,
                  values=tf.math.floordiv(embedding_tensor.values, R_vocab_size),
                  dense_shape=embedding_tensor.dense_shape)
            ...
            if isinstance(feature_column, FeatureColumn3D):
              ...
              example["{}_0_row_lengths".format(feature_column.fc_name)] = row_lengths
              # seq feature, dims[0][0] is max seq length
              new_embedding_tensor = tf.sparse.slice(
                new_embedding_tensor, [0, 0], [new_embedding_tensor.dense_shape[0], feature_column.max_seq_length]
              )

            example["{}_0".format(feature_column.fc_name)] = tf.sparse.reorder(new_embedding_tensor)

            for feature_slice in feature_column.feature_slice_to_tf_placeholder:
              if feature_slice.slice_index != 0:
                example["{}_{}".format(feature_column.fc_name, feature_slice.slice_index)] = example["{}_0".format(feature_column.fc_name)]
        ...
        for feature_column in feature_slot.feature_columns:
          for feature_slice in feature_column.feature_slice_to_tf_placeholder:
            if self.p.qr_multi_hashing and vocab_size > self.p.qr_hashing_threshold:
              ...
              table_name = "table_{}_{}_0".format(slot_id, feature_slice.slice_index)
              if table_name not in self._table_to_config_dict:
                Rtable = tpu_embedding.TableConfig(vocabulary_size=R_vocab_size,
              ...
              feature_name = "{}_{}_0".format(feature_column.fc_name, feature_slice.slice_index)
              self._feature_to_config_dict[feature_name] = tpu_embedding.FeatureConfig(
                  table_name)


              # quotient embedding table
              table_name = "table_{}_{}_1".format(slot_id, feature_slice.slice_index)
              if table_name not in self._table_to_config_dict:
                Qtable = tpu_embedding.TableConfig(vocabulary_size=Q_vocab_size,
                                                  dimension=feature_slice.dim,
                                                  initializer=feature_slice.initializer,
                                                  combiner="sum",
                                                  learning_rate_fn=feature_slice.learning_rate_fn,
              ...
              feature_name = "{}_{}_1".format(feature_column.fc_name, feature_slice.slice_index)
              self._feature_to_config_dict[feature_name] = tpu_embedding.FeatureConfig(
                  table_name)


            table_name = "table_{}_{}".format(slot_id, feature_slice.slice_index)
            if table_name not in self._table_to_config_dict:
            ...
            feature_name = "{}_{}".format(feature_column.fc_name, feature_slice.slice_index)
            # Multiple feature configs can share the same table config
            if isinstance(feature_column, FeatureColumn3D):
              self._feature_to_config_dict[feature_name] = tpu_embedding.FeatureConfig(
                table_name, max_sequence_length=feature_column.max_seq_length)
            else:
              self._feature_to_config_dict[feature_name] = tpu_embedding.FeatureConfig(
                table_name)
