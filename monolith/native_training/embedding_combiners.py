...
class ReduceSum(Combiner):
  ...
  def combine(self,
              ...
              embedding: tf.Tensor,
              name: str = None):
    return distribution_ops.reduce_sum(tf.expand_dims(ragged_utils.fused_value_rowids(key), -1),
                                       embedding,
...
class ReduceMean(Combiner):
  ...
  def combine(self,
              ...
              embedding: tf.Tensor,
              name: str = None):
    return distribution_ops.reduce_mean(tf.expand_dims(ragged_utils.fused_value_rowids(key), -1),
                                        embedding,
                                        tf.expand_dims(key.nrows(), 0),
                                        name=name)
