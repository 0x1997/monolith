...
class NativeContext:
  ...
  def __init__(self,
    ...
    if layout_factory and feature_factory:
      raise ValueError("Cannot set feature_factory and layout_factory in the same time")
...
class NativeTask(BaseTask, abc.ABC):
  ...
  def params(cls):
    ...
    p.metrics.define('extra_fields_keys', [], 'extra_fields_keys for deepinsight, List[str]')
    ...
    p.metrics.define("enable_kafka_metrics", False, 
                     "enable_kafka_metrics")
    p.metrics.define(
    ...
    p.serving.define("export_with_gpu_allowed", False, 
        "When true it allows cpu/gpu training to export model graph "
        "with specified gpu device placement contexts.")
    p.serving.define("export_with_cleared_entry_devices", False, 
