...
def organize_valid_indices(is_valid, shuffle=True, seed=None):
  ...
  return array_ops.concat(
      [array_ops.expand_dims(batch_ids, 2),  #[[0,...0], [1, ..., 1]]
       array_ops.expand_dims(indices, 2)],  # shuffle之后的indices，dim0=batch  dim1=shuffle后的index,例如[0, 1, 2, 3] 变为[2,3,0,1]，为list的长度
      axis=2)
...
def get_batch_idx_size(logits, labels, rank_id, name_prefix):
  ...
  range_count_tile = tf.tile(tf.expand_dims(tf.range(unique_rank_id_num), 1), [1, batch_size])
  ...
  row_cor = rank_idx
  batch_idx = tf.concat([tf.expand_dims(row_cor, 1), tf.expand_dims(col_cor, 1)], axis=1)
  output_shape = [tf.shape(count)[0], max_count]

  logits_idx = tf.scatter_nd(updates=logits, indices=batch_idx, shape=output_shape,
                              name=name_prefix + "logits_idx")
  label_idx = tf.scatter_nd(updates=labels, indices=batch_idx, shape=output_shape,
                            name=name_prefix + "label_idx") - 1e-6
  mask_idx = tf.scatter_nd(updates=tf.ones_like(logits), indices=batch_idx, shape=[batch_size, batch_size],
  ...
  def individual_weights(self, sorted_labels):
    ...
    with ops.name_scope(None, 'dcg_lambda_weight', (sorted_labels,)):
      ...
      sorted_labels = array_ops.where(
          label_valid_fn(sorted_labels), sorted_labels,
          array_ops.zeros_like(sorted_labels))
      gain = self._gain_fn(sorted_labels)
      if self._normalized:
        gain *= inverse_max_dcg(
...
def _pairwise_comparison(sorted_labels,
  ...
  * `pairwise_weights` = | |l_i - l_j|    if lambda_weight is None,
  ...
  pairwise_weights *= tf.cast(array_ops.expand_dims(sorted_weights, 2), tf.float32)
...
def _pairwise_hinge_loss(labels,
  ...
  with ops.name_scope(name, 'pairwise_hinge_loss', (labels, logits, weights)):
    return _pairwise_loss(
        _loss, labels, logits, weights, lambda_weight, lambda_scale, reduction=reduction)
