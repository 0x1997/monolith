...
features = {
    'f_user_id': FeatMeta(name='f_user_id', slot=1, max_sequence_length=1, 
                          slice_dims=table1.slice_dims, table=table1.name, pool_type=1, fl_type=1),
    'f_user_ctx_network': FeatMeta(name='f_user_ctx_network', slot=61, max_sequence_length=1, 
                                   slice_dims=table1.slice_dims, table=table1.name, pool_type=1, fl_type=1),
    'f_user_test10_array': FeatMeta(name='f_user_test10_array', slot=549, max_sequence_length=10, 
                                    slice_dims=table1.slice_dims, table=table1.name, pool_type=2, fl_type=1),
    'f_user_id-f_page': FeatMeta(name='f_user_id-f_page', slot=504, max_sequence_length=10, fid_version=1,
                                 slice_dims=table3.slice_dims, table=table3.name, pool_type=3, fl_type=1),
    'f_goods_id': FeatMeta(name='f_user_id', slot=200, max_sequence_length=1, 
                           slice_dims=table2.slice_dims, table=table2.name, pool_type=1, fl_type=0),
    'f_page': FeatMeta(name='f_page', slot=305, max_sequence_length=1, 
                       slice_dims=table2.slice_dims, table=table2.name, pool_type=1, fl_type=0),
}
    ...
    for name, meta in features.items():
      ...
      for i in range(batch_size):
        ...
        if meta.fid_version == 0:
          mask = (1 << 54) - 1
          feature.fid_v1_list.value.extend([(meta.slot << 54) | (randint(1, mask) & mask) 
            for _ in range(meta.max_sequence_length)])
        else:
          mask = (1 << 48) - 1
          feature.fid_v2_list.value.extend([(meta.slot << 48) | (randint(1, mask) & mask) 
            for _ in range(meta.max_sequence_length)])
