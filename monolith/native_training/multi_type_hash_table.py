...
class MultiTypeHashTable(BaseMultiTypeHashTable):
  ...
  def fused_apply_gradient(
    ...
    copied = copy.copy(self)
    updated_tables = dict(self._hash_tables)
    for slot, handle in zip(
        sorted(self._slot_to_config.keys()), table_handles_output):
...
class MergedMultiTypeHashTable(BaseMultiTypeHashTable):
  ...
  def __init__(self, slot_to_config: Dict[str, entry.HashTableConfigInstance],
               factory: Callable[[Dict[str, entry.HashTableConfigInstance]],
                                 BaseMultiTypeHashTable]):
    self._slot_to_config = slot_to_config
    logging.info("Create MergedMultiTypeHashTable: 1) reverse feature_name -> config into config -> feature_name_list")
    ...
    logging.info("Create MergedMultiTypeHashTable: 2) gen merged_slot and map merged_slot -> config")
    ...
    logging.info(f"Create MergedMultiTypeHashTable: 3) sub hash tables {factory}")
  ...
  def lookup(self,
    ...
    if early_reorder_indicies_res_pack:
      merged_slot_to_id, merged_slot_to_sizes, res_pack = early_reorder_indicies_res_pack
      ...
      logging.info("Lookup MergedMultiTypeHashTable: 1) merged the features ids belong the same hash table")
      merged_slot_to_id, merged_slot_to_sizes = self._get_merged_to_indexed_tensor(
          slot_to_id)
      auxiliary_bundle['merged_slot_to_sizes'] = merged_slot_to_sizes
      logging.info(f"Lookup MergedMultiTypeHashTable: 2) lookup sub hash table {self._table} for embeddings")
    ...
    logging.info("Lookup MergedMultiTypeHashTable: 3) split the lookuped embeddings into feature_name -> embedding")
    slot_to_embedding = {}
  ...
  def get_table_dim_sizes(self) -> List[int]:
    return [
        infer_dim_size(
            self._merged_slot_to_config[slot].table_config)
