...
def _device_rule(device_name):
  ...
  if (d.device_type == "GPU" and not _GPU_PLACEMENT_ALLOWED) or not d.device_type:
...
def default_device_fn(op):
  """Default device_fn for Estimator RunConfig."""
  # TODO(Peng): make it default for all Estimator RunConfig (including async training)
  if op.device:
