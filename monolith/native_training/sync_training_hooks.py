...
class SyncTrainingBarrierSaverListener(tf.estimator.CheckpointSaverListener):
  ...
  def begin(self):
    ...
    if enable_hvd != None:
      self._barrier_var = tf.compat.v1.placeholder(
        dtype=tf.int64, shape=[], name="hvd_export_barrier_ph")
  ...
  def after_save(self, session, global_step_value):
    if enable_hvd:
      ...
      try:
        barrier_val = session.run(self._barrier_op, feed_dict={self._barrier_var: global_step_value})
...
class ParameterSyncHook(session_run_hook.SessionRunHook):
  ...
  def __init__(self, sync_backend, ps_index, refresh_interval=100):
    ...
    logging.info(f"sync hook for ps_{self._ps_index} with refresh_interval={self._refresh_interval}")

  def begin(self):
    self._config_ph = tf.compat.v1.placeholder(tf.string, shape=(), name="sync_config_str")
  ...
  def before_run(self, run_context):
    ...
    if cur_time - self._last_refresh_time >= self._refresh_interval:
      self._sync_config = refresh_sync_config(self._sync_backend, self._ps_index)
...
class SyncTrainingForceDumpHook(tf.estimator.SessionRunHook):
  ...
  def begin(self):
    ...
    self._ctrl_ph = tf.compat.v1.placeholder(tf.int16, shape=(3,), name='hvd_dump_ctrl')
...
class SyncTrainingRefreshTokenHook(tf.estimator.SessionRunHook):
  
  def __init__(self, refresh_interval=3600):
    self._refresh_interval = refresh_interval
    self._token_path = os.path.join(find_main(), f"sec_token-{uuid.uuid4()}.txt")
    self._token_issuer = token_utils.TokenIssuer()
    self._last_refresh_time = 0
    self.refresh_token()
    os.environ['SEC_TOKEN_PATH'] = self._token_path
    logging.info(f"using SEC_TOKEN_PATH={self._token_path}")
  ...
  def refresh_token(self):
    issuer_user = os.environ.get('MONOLITH_TOKEN_ISSUER') or os.environ['USER']
    cur_time = int(time.time())
    if cur_time - self._last_refresh_time > self._refresh_interval:
      with tempfile.NamedTemporaryFile(mode='w', dir=find_main(), delete=False) as f: 
        f.write(self._token_issuer.issue_token(issuer_user))   
        tmp_token_path = f.name
      logging.info(f"replacing {self._token_path} with {tmp_token_path} at {cur_time}") 
      os.replace(tmp_token_path, self._token_path)  
      self._last_refresh_time = cur_time
      logging.info("gdpr token refreshed")
...
class ReqTimeControlDumpHook(tf.estimator.SessionRunHook):
  ...
  def begin(self):
    ...
    self._req_time_ph = tf.compat.v1.placeholder(tf.int64, shape=[2], name="hvd_req_time")
    self._req_time_bcast_op = mpi_ops.broadcast(self._req_time_ph, 0)
  
  def before_run(self, run_context):
    if hvd.rank() == 0:
      return session_run_hook.SessionRunArgs(fetches={'req_time': self._req_time})
    else:
      return None
