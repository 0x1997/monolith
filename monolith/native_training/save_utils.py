...
class PartialRecoverySaver():
  ...
  * `keep_checkpoint_every_n_hours`: In addition to keeping the most recent
  ...
  def _RecordLastCheckpoint(self, latest_save_path):
    ...
    exempt_checkpoint_paths = last_checkpoint_paths & self._exempt_checkpoint_paths
    if len(self._last_checkpoints) - len(
      ...
      for p in self._last_checkpoints:
        ...
        if filename not in self._exempt_checkpoint_paths:
  ...
  def restore(self, sess, save_path):
    ...
    if init_objs:
      ...
      if alias_map:  # alias_map: old -> new
