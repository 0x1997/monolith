...
def apply_gradients(ctx: NativeContext, 
  ...
  if variables:
    ...
    train_ops.append(var_opt.apply_gradients(grads_and_vars, global_step=global_step))
  else:
    with tf.control_dependencies([training_util._get_or_create_global_step_read()]):
