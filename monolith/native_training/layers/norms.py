...
class BatchNorm(Layer):
  ...
  def call(self, inputs, **kwargs):
    ...
    if self.mode == tf.estimator.ModeKeys.TRAIN:
      ...
      add_layer_loss('{}_moving_mean'.format(self.name), tf.reduce_sum(moving_mean))
      add_layer_loss('{}_moving_variance'.format(self.name), tf.reduce_sum(moving_variance))
      ...
      tf.compat.v1.summary.scalar(self.name + '/mean_train', tf.reduce_mean(mean))
      tf.compat.v1.summary.histogram(self.name + '/var_train', variance)
      tf.compat.v1.summary.scalar(self.name + '/var_train', tf.reduce_mean(variance))
      ...
      tf.compat.v1.summary.scalar(self.name + '/mean_test', tf.reduce_mean(mean))
      tf.compat.v1.summary.histogram(self.name + '/var_test', variance)
      tf.compat.v1.summary.scalar(self.name + '/var_test', tf.reduce_mean(variance))
...
class GradNorm(Layer):
  ...
  def build(self, input_shape):
    ...
    for i in range(n):
      tf.compat.v1.summary.scalar('gradnorm_weight/{}'.format(self.loss_names[i]),
                        self._weights[i])
  ...
  def call(self, inputs, **kwargs):
    ...
    for i in range(n):
      tf.compat.v1.summary.scalar('gradnorm_gnorm/{}'.format(self.loss_names[i]),
                        gnorms[i])
      tf.compat.v1.summary.scalar('gradnorm_wgnorm/{}'.format(self.loss_names[i]),
                        wgnorms[i])
