...
class GroupInt(Layer):
  ...
  def call(self, inputs, **kwargs):
    ...
    ffm_embeddings = ffm(left=left, right=right, dim_size=last_dim_size,
    ...
    if self.allow_kernel_norm:
      ...
      with tf.compat.v1.name_scope(f'{upper_ns}/{name}/') as name_scope:
        ...
        normalized = tf.nn.l2_normalize(var,
                                        ...
                                        epsilon=1e-6,
                                        name='normalized_var')
        var_norm_init = tf.norm(var_init,
                                axis=0,
                                name='init_trainable_norm')
        ...
        with tf.compat.v1.variable_scope('', reuse=tf.compat.v1.AUTO_REUSE):
          trainable_var_norm = tf.compat.v1.get_variable(initializer=var_norm_init,
                                                          name=f'{var_name}/trainable_norm',
                                                          dtype=dtype)

        if base_layer_utils.is_split_variable(trainable_var_norm) or isinstance(trainable_var_norm, variable_ops.PartitionedVariable):
