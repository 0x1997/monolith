...
class DIN(Layer):
  ...
  def call(self, inputs, **kwargs):
    ...
    if mask is not None:
      ...
      tf.compat.v1.summary.histogram('{name}_attention_outputs'.format(name=self.name),
                           attention_weight)
...
class DMR_U2I(Layer):
  ...
  def call(self, inputs, **kwargs):
    ...
    user_seq_merged = tf.squeeze(tf.matmul(user_seq_trans,
                                           alpha), axis=-1)  # (bs, ue_size)
