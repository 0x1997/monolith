...
class LHUCTower(Layer):
  ...
  def __init__(self,
               ...
               **kwargs):
    self._lhuc_kwargs = {k: v for k, v in kwargs.items() if k.startswith('lhuc_')}
    ...
    if activations is None:
      ...
      self.activations = [ad_acts.get(activations) if i != self.n_layers -1 else None 
    ...
    if lhuc_output_dims:
      ...
      if all(isinstance(dims, (list, tuple)) for dims in lhuc_output_dims):
        for i, dims in enumerate(lhuc_output_dims):
          assert dims[-1] == output_dims[i], "the last dim of lhuc must be identity with dense output"
    ...
    self.lhuc_activations = [[ad_acts.get('relu') if i != len(dims) -1 else ad_acts.get('sigmoid2') 
                              for i in range(len(dims))] for dims in self.lhuc_output_dims]
  ...
  def build(self, input_shape):
    if self.enable_batch_normalization:
      bn_layer = BatchNorm(name='batch_norm',
    ...
    for i, dim in enumerate(self.output_dims):
      ...
      if i != (self.n_layers - 1) and self.enable_batch_normalization:
        bn_layer = BatchNorm(name=f'{layer_name}/batch_norm',
                             momentum=self.batch_normalization_momentum,
                             renorm=self.batch_normalization_renorm,
                             renorm_clipping=self.batch_normalization_renorm_clipping,
                             renorm_momentum=self.batch_normalization_renorm_momentum)
      ...
      mlp = MLP(name=f'{layer_name}/lhuc',
                ...
                use_learnable_weight_norm=self.lhuc_params('use_learnable_weight_norm'),
                use_bias=self.lhuc_params('use_bias'),
                bias_regularizer=self.lhuc_params('bias_regularizer'),
                enable_batch_normalization=self.lhuc_params('enable_batch_normalization'),
                batch_normalization_momentum=self.lhuc_params('batch_normalization_momentum'),
                batch_normalization_renorm=self.lhuc_params('batch_normalization_renorm'),
                batch_normalization_renorm_clipping=self.lhuc_params('batch_normalization_renorm_clipping'),
                batch_normalization_renorm_momentum=self.lhuc_params('batch_normalization_renorm_momentum'))
