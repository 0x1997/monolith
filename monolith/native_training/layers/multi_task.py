...
class MMoE(Layer):
  ...
  def __init__(self,
    ...
    if isinstance(expert_activations, (tuple, list)):
      ...
      self._expert_activations = [activations.get(act) 
                                  for act in expert_activations]
    else:
      self._expert_activations = [activations.get(expert_activations) 
    ...
    if isinstance(expert_initializers, (tuple, list)):
      ...
      self._expert_initializers = [initializers.get(init) 
                                   for init in expert_initializers]
    else:
      self._expert_initializers = [initializers.get(expert_initializers) 
                                   for _ in range(self._num_experts)]
  ...
  def build(self, input_shape):
    ...
    elif all(isinstance(shape, tf.compat.v1.Dimension) for shape in input_shape):
    ...
    if self._gate_type == 'noise_topk':
      self._gate_noise = self.add_weight(name="gate_noise", 
                                         shape=gate_shape,
                                         dtype=tf.float32,
  ...
  def calc_gate(self, gate_input: tf.Tensor):
    ...
    # (batch, num_tasks, num_experts)
    gete_logit = tf.reshape(gete_logit, shape=(-1, self._num_tasks, self._num_experts))
    ...
    if self._gate_type in {'topk', 'noise_topk'}:
      ...
      # (batch, num_tasks, num_experts)
      gates = tf.where(gates >= threshold, gates, tf.zeros_like(gates, dtype=gates.dtype))
  ...
  def call(self, inputs, **kwargs):
    ...
    expert_outputs = tf.stack([expert(expert_input) for expert in self.experts], axis=2)
  ...
  def get_config(self):
    config = {
      ...
      'expert_output_dims': self._expert_output_dims,
      "expert_activations": [ad_acts.serialize(act) for act in self._expert_activations],
      "expert_initializers": [
          tf.initializers.serialize(init) for init in self._expert_initializers
      ],
...
class SNR(Layer):
  ...
  def build(self, input_shape):
    ...
    if self._snr_type == 'trans':
      ...
      self._weight = tf.tile(tf.reshape(tf.eye(self._in_subnet_dim), shape=(1, block_size)), 
  ...
  def sample(self):
    if self._mode != tf.estimator.ModeKeys.PREDICT:
      ...
      s = tf.sigmoid((tf.math.log(u) - tf.math.log(1.0 - u) + self._log_alpha) / self._beta)
    else:
  ...
  def call(self, inputs, **kwargs):
    ...
    shape1 = (self._num_in_subnet, self._num_out_subnet, self._in_subnet_dim, self._out_subnet_dim)
    shape2 = (self._num_in_subnet * self._in_subnet_dim, self._num_out_subnet * self._out_subnet_dim)
    weight = tf.reshape(tf.transpose(tf.reshape(weight, shape1), perm=[0, 2, 1, 3]), shape2)

    return tf.split(tf.matmul(tf.concat(inputs, axis=1), weight), 
                    num_or_size_splits=self._num_out_subnet, axis=1)
