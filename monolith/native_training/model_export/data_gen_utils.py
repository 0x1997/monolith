...
class ParserArgs(object):
  ...
  def __post_init__(self):
    ...
    self.dense_feature_shapes = self.dense_feature_shapes or self.get('dense_feature_shapes')
    self.dense_feature_types = self.dense_feature_types or self.get('dense_feature_types')
    self.extra_features = self.extra_features or self.get('extra_features')
    self.extra_feature_shapes = self.extra_feature_shapes or self.get('extra_feature_shapes')
...
def gen_fids_v1(slot: int, size: int = 1) -> List[int]:
  assert 0 < slot < MAX_SLOT_V1
  return [(slot << 54) | (randint(1, sys.maxsize) & MASK_V1) for _ in range(size)]


def gen_fids_v2(slot: int, size: int = 1) -> List[int]:
  assert 0 < slot < MAX_SLOT_V2
  return [(slot << 48) | (randint(1, sys.maxsize) & MASK_V2) for _ in range(size)]
...
def _(feature: EFeature, meta: FeatureMeta, drop_rate: float = 0):
  ...
  if size == -1:  # sparse 
    if '_recent' in name or '_cp' in name or feat.method in {'Combine', 'VectorTopString'}:
...
def fill_line_id(line_id, features: List[FeatureMeta] = None, 
                 hash_len: int = 48, actions: List[int] = None):
  MASK = MASK_V1 if hash_len == 54 else MASK_V2
  if features:
    for meta in features:
      name, shape = meta.name, meta.shape
      if name == 'uid':
        line_id.uid = (1 << hash_len) | (randint(sys.maxsize // 2, sys.maxsize) & MASK)
      elif name == 'item_id':
        line_id.item_id = (200 << hash_len) | (randint(sys.maxsize // 2, sys.maxsize) & MASK)
      elif name == 'req_time':
        ...
        if desc.label == FieldDescriptor.LABEL_REPEATED:
          ...
          if desc.cpp_type in {FieldDescriptor.CPPTYPE_DOUBLE, FieldDescriptor.CPPTYPE_FLOAT}:
            value_list.extend([uniform(0, 1) for _ in range(shape)])
          elif desc.cpp_type in {FieldDescriptor.CPPTYPE_INT32,
...
def sort_header(sort_id: bool, kafka_dump: bool, kafka_dump_prefix: bool):
  ...
  if sort_id and not (kafka_dump or kafka_dump_prefix):
    ...
    raise Exception('kafka_dump_prefix={kafka_dump_prefix}, sort_id={sort_id}, kafka_dump={kafka_dump} not support')
...
def gen_example(sparse_features: List[str],
  ...
  assert len(sparse_features) > 0 and len(sparse_features) == len(set(sparse_features))
  ...
  for name in sparse_features:
    try:
      ...
      if feat is not None:
        name_to_info[name] = FeatureMeta(name, slot=feat.slot, dtype=tf.int64, extra=feat)
    except:
  ...
  if label_meta:
    example.label.extend([choice([0, 1]) 
      for _ in range(label_meta.shape)])
  else:
...
def gen_instance(fidv1_features: List[int] = None,
  ...
  if fidv1_features is not None:
    assert len(fidv1_features) > 0 and len(fidv1_features) == len(set(fidv1_features))
    for slot in fidv1_features:
      size = 1 if slot in {1, 200} else randint(0, 3)
      inst.fid.extend(gen_fids_v1(slot, size))

  name_to_info = {}
  if fidv2_features:
    assert len(fidv2_features) > 0 and len(fidv2_features) == len(set(fidv2_features))
...
def gen_example_batch(sparse_features: List[str],
  ...
  assert len(sparse_features) > 0 and len(sparse_features) == len(set(sparse_features)) and batch_size > 0
  name_to_info = {}
  for name in sparse_features:
    try:
      feat = feature_list.get(name)
  ...
  for i in range(batch_size):
    feature = named_feature_list.feature.add()
    if label_meta:
      feature.float_list.value.extend([choice([0, 1]) 
        for _ in range(label_meta.shape)])
    else:
...
def gen_prediction_log(args: ParserArgs) -> Iterable[prediction_log_pb2.PredictionLog]:
  assert args.variant_type in {'example', 'instance', 'example_batch', 'examplebatch'} and args.batch_size < args.max_records
  ...
  if args.extra_features:
    extra_meta = [FeatureMeta(name=name, shape=shape)
                  for name , shape in zip(args.extra_features,
                                          args.extra_feature_shapes)]
  ...
  if export_ctx is None:
    ...
    signatures = {signature.name: signature for signature in export_ctx.signatures(graph)}
  ...
  for i in range(num_log):
    ...
    if signatures is None or signatures[signature_name].inputs:
      ...
      if args.variant_type == 'example':
        instances = [gen_example(args.sparse_features,
                                dense_feature_meta, extra_meta,
                                args.feature_list,
                                args.drop_rate).SerializeToString()
                    for _ in range(args.batch_size)]
      elif args.variant_type == 'instance':
        instances = [gen_instance(args.fidv1_features, args.fidv2_features,
                                  dense_feature_meta, extra_meta,
                                  args.feature_list,
                                  args.drop_rate).SerializeToString()
                    for _ in range(args.batch_size)]
      else:
        instances = [gen_example_batch(args.sparse_features, dense_feature_meta,
                                      extra_meta, args.feature_list,
                                      args.batch_size,
                                      args.drop_rate).SerializeToString()]
...
def gen_warmup_file(warmup_file: str = None, drop_rate: float = None):
  ...
  if not warmup_args.warmup_file:
    ...
    try:
      logging.info(f'begin to write prediction log to {warmup_args.warmup_file}')
      ...
      logging.info(f'finish to write prediction log to {warmup_args.warmup_file}')
      return warmup_args.warmup_file
...
def gen_random_data_file(data_file_name: str,
  ...
  for i in range(num_batch):
    if args.variant_type == 'example':
      instances.extend([gen_example(args.sparse_features, dense_feature_meta,
                                    extra_meta, args.feature_list,
                                    args.drop_rate,
                                    actions=actions).SerializeToString()
                        for _ in range(args.batch_size)])
    elif args.variant_type == 'instance':
      instances.extend([gen_instance(args.fidv1_features, args.fidv2_features,
                                     dense_feature_meta, extra_meta,
                                     args.feature_list, args.drop_rate,
                                     actions=actions).SerializeToString()
                        for _ in range(args.batch_size)])
    else:
      instances.extend([gen_example_batch(args.sparse_features, dense_feature_meta,
                                          extra_meta, args.feature_list,
                                          args.batch_size, args.drop_rate,
                                          actions=actions).SerializeToString()])
  if sort_id:
