...
#
# example usage:
# cat ~/tmp/monolith_native_training_tasks_iptv_push_iptv_push.pb | \
# bazel run //monolith/native_training/model_export:warmup_data_gen -- \
# --batch_size=256 --kafka_dump_prefix=false --has_sort_id=true --kafka_dump=false \
# --output_path=/tmp/warmup_data_prediction
#
import sys
...
flags.DEFINE_string("output_path", "/tmp/tf_warmup_data", "output path of warmup data.")
flags.DEFINE_enum("variant_type", "instance", ['instance', 'example', 'example_batch'],
                  "variant_type")
...
def gen_prediction_log_from_file(file_name: str = None,
  ...
  for i, batch in enumerate(reader):
    ...
    request.model_spec.signature_name = signature_names[i % len(signature_names)]
...
def main(_):
  ...
  with tf.io.TFRecordWriter(FLAGS.output_path) as writer:
    if FLAGS.gen_type == 'file':
      for log in gen_prediction_log_from_file(FLAGS.file_name, FLAGS.batch_size,
                                              FLAGS.lagrangex_header, FLAGS.kafka_dump_prefix, FLAGS.has_sort_id,
                                              FLAGS.kafka_dump, FLAGS.max_records, FLAGS.variant_type):
      ...
      if FLAGS.dense_features is not None:
        ...
        dense_feature_shapes = [int(shape) for shape in FLAGS.dense_feature_shapes.split(',')]
        dense_feature_types = [tf_dtype(dtype) for dtype in FLAGS.dense_feature_types.split(',')]
      else:
      ...
      if FLAGS.extra_features is not None:
        ...
        extra_feature_shapes = [int(shape) for shape in FLAGS.extra_feature_shapes.split(',')]
      else:
      ...
      for log in gen_prediction_log(FLAGS.model_name, sparse_features, 
                                    dense_features, dense_feature_shapes, dense_feature_types, 
                                    extra_features, extra_feature_shapes, 
                                    feature_list, FLAGS.batch_size, FLAGS.max_records, None, 
