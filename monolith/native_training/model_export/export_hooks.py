...
class ExportSaverListener(tf.estimator.CheckpointSaverListener):
  ...
  def _add_entry_to_state(self, export_dir: bytes, global_step_value: int):
    ...
    state = get_export_saver_listener_state(export_dir_base)
  ...
  def _maybe_delete_old_entries(self, export_dir: bytes):
    export_dir = export_dir.decode()
    export_dir_base = os.path.dirname(export_dir)
    old_state = get_export_saver_listener_state(export_dir_base)
    existing_steps = self._helper.get_existing_checkpoint_steps() | self._exempt_checkpoint_steps
    if self._dense_only:
      path = Path(export_dir_base)
      model_dir = str(path.parent.parent)
      full_stats = tf.train.get_checkpoint_state(model_dir)
      if full_stats:
        existing_steps |= set([get_global_step(ckpt) for ckpt in full_stats.all_model_checkpoint_paths])
    ...
    overwrite_export_saver_listener_state(export_dir_base, new_state)
