...
def eigen_inverse_root(mat, p, head, tail, damping = 1e-3):
    ...
    zeros = tf.cond(tf.greater(tf.size(non_zero), 0),
                    lambda: tf.cast(tf.reduce_min(non_zero), dtype = "int32"),
...
class ShampooOptimizer(tf.compat.v1.train.Optimizer):
    ...
    def _create_slots(self, var_list):
        ...
        for var in var_list:
            for i, dim in enumerate(var.shape):
                ...
                self._get_or_make_slot(var, tf.zeros([dim, dim]), "s" + str(i), self._name + "/s" + str(i))
                self._get_or_make_slot(var, tf.zeros([dim, dim]), "g" + str(i), self._name + "/g" + str(i))
                self._get_or_make_slot(var, tf.zeros([dim, eigens]), "pvec" + str(i), self._name + "/pvec" + str(i))
                self._get_or_make_slot(var, tf.zeros([eigens]), "pval" + str(i), self._name + "/pval" + str(i))
                self._get_or_make_slot(var, tf.zeros([]), "o" + str(i), self._name + "/o" + str(i))
    ...
    def _resource_apply_dense(self, grad, var):
        ...
        if_update_precond = tf.math.logical_and(
                                if_warmed_up,
                                tf.equal(tf.math.mod(global_step, tau_1), 0))
        
        global_step_f = tf.cast(global_step, dtype = 'float32')
        warmup_f = tf.cast(self._warmup, dtype = 'float32')
        warmup_rate = tf.minimum(tf.maximum(global_step_f / warmup_f - 1.0, 0.0), 1.0)

        if_stat_momentum = tf.less(beta_2, 1.0 - 1e-10) # if beta_2 = 1.0, do not use momentum on statistics
        ...
        for i in range(rank):
            ...
            g_t = tf.cond(if_update_stat,
                lambda: state_ops.assign(g, tf.tensordot(grad, grad, axes = [axes, axes])),
                lambda: tf.identity(g))
            
            s = self.get_slot(var, 's' + str(i))
            s_t = tf.cond(if_stat_momentum,
            ...
            def update_precond():
                pvec_t, pval_t, offset_t = eigen_inverse_root(s_t, 2 * rank, eigen_head, eigen_tail, damping_epsilon)
                return (state_ops.assign(pvec, pvec_t),
                        state_ops.assign(pval, pval_t),
                        state_ops.assign(offset, offset_t))
            
            pvec_t, pval_t, offset_t = tf.cond(if_update_precond,
                lambda: update_precond(),
                lambda: (tf.identity(pvec), tf.identity(pval), tf.identity(offset)))
            
            grad_precond = apply_sparse_precond(grad_precond, pvec_t, pval_t, offset_t)
            ops += [g_t, s_t, pvec_t, pval_t, offset_t,]
            
        d = self.get_slot(var, 'd')
        d_t = state_ops.assign_add(d, grad * grad)
        
        m = self.get_slot(var, 'm')
        m_t = state_ops.assign(m, beta_1 * m + (1 - beta_1) * grad * tf.math.rsqrt(d_t + 1e-30))
        ...
        var_t = tf.cond(if_warmed_up,
                        lambda: state_ops.assign_sub(var, (1.0 - warmup_rate) * update_diag + warmup_rate * update_second),
