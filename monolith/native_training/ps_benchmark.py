  ...
  def end(self, sess):
    ...
    if self._config.index == 0 and not self._config.ps_str_overridden:
      ...
      reversed_sorted_throughput_and_ps = sorted(
          [[throughput, i, self._config.ps_list[i].split(":")[0]] for i, throughput in enumerate(throughput_value)])
      sorted_throughput_and_ps = [item[:] for item in reversed_sorted_throughput_and_ps]
      ...
      for i in range(len(reversed_sorted_throughput_and_ps) - 1):
        for j in range(i + 1, len(reversed_sorted_throughput_and_ps)):
          if reversed_sorted_throughput_and_ps[i][2] == reversed_sorted_throughput_and_ps[j][2]:
            sorted_throughput_and_ps[j][0] += reversed_sorted_throughput_and_ps[i][0]
      
      sorted_throughput_and_ps = sorted(sorted_throughput_and_ps, reverse=True)
  
      logging.info("Measure result (throughput, ps) (ps with the same ip addresses had their throughput adjusted): {}".format([
          "ps_{}({}):{}".format(ps, ip, throughput)
