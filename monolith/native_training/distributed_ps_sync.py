...
enable_bps_bwd_fake_cast = int(os.getenv("MONOLITH_WITH_BYTEPS_BWD_FAKE_CAST", "0"))
...
class DistributedMultiTypeHashTableMpi(
  ...
  def lookup(self,
    ...
    if FLAGS.enable_alltoall_metrics:
      tf.compat.v1.summary.histogram("fused_embedding_splits", embedding_splits)
    ...
    auxiliary_bundle["recv_emb_splits"] = tf.reshape(
      tf.matmul(
        ...
        tf.expand_dims(tf.constant(self._output_dims, dtype=tf.int32), -1)  # [slot_num, 1]
      ), [-1]  # flatten
    )
    ...
    if enable_bps and enable_bps_fwd:
      if enable_bps_fwd_gdr:
        if enable_bps_fwd_gdr_g2g:
          ...
          with tf.device("/device:GPU:0"):
            recv_embeddings = bps.alltoall(
              fused_embeddings_gpu,
              embedding_splits,
      ...
      if enable_hvd_fwd_g2g:
        ...
        with tf.device("/device:GPU:0"):
          recv_embeddings = hvd.alltoall(fused_embeddings, embedding_splits, name='hvd_fwd_a2a_g2g')
      else:
        logging.info('Enabled hvd for fwd embed alltoall')
        recv_embeddings = hvd.alltoall(fused_embeddings, embedding_splits, name='hvd_fwd_a2a')

    auxiliary_bundle["recv_embeddings"]  = recv_embeddings

    with tf.device("/device:GPU:0"):
      # GPUQueue: Pass to GPU at Enqueue
      auxiliary_bundle["recv_embeddings"] = tf.identity(auxiliary_bundle["recv_embeddings"])
    ...
    # Keep on CPU  
    auxiliary_bundle["fused_embedding_offsets"] = [tf.identity(t) for t in list(auxiliary_bundle["fused_embedding_offsets"])]
  ...
  def apply_gradients(
    ...
    sorted_slot_keys = sorted(slot_to_id_and_grad.keys())
    sorted_grads = [slot_to_id_and_grad[k][1] for k in sorted_slot_keys]
    ...
    if enable_bps and enable_bps_bwd:
      ...
      if enable_bps_bwd_gdr:
        ...
        logging.info('Enabled BPS for bwd embed alltoall with cast optimization')
        grad_flat_t = bps.alltoall(grad_flat,
                                   recv_emb_splits,
                                   recv_splits=embedding_splits,
                                   name="bwd_alltoall")
      ...
      if enable_hvd_bwd_g2g:
        ...
        with tf.device("/device:GPU:0"):
          grad_flat_t = hvd.alltoall(grad_flat, recv_emb_splits, name='hvd_bwd_a2a_g2g')
          with tf.device("/device:CPU:0"): 
            grad_flat_t = tf.identity(grad_flat_t)
      else:
    ...
    updated_table = self._table.fused_apply_gradient(
          auxiliary_bundle.pop("id_flat_t"), auxiliary_bundle.pop("id_size_flat_t"), auxiliary_bundle.pop("grad_flat_t"),
    ...
    if async_optimize_queue:
      self.add_queue_hook(AsyncPushHook(async_optimize_queue, update_op.as_op()))
