...
def run_fused_reorder_by_indicies(suffices=None):
  ...
  ids = list(set(np.random.randint(low=int64.min, high=int64.max+1, size=num_ids)))
  ...
  with tf.compat.v1.Session(config=session_config) as sess:
    ...
    reorder_op = distribution_ops.fused_reorder_by_indices(ids_list, num_of_shards=num_of_shards)
    start = time.time()
...
if __name__ == "__main__":
  ...
  # np.random.seed(1234)
  print('> Sess.run Wall Time:', np.average([run_fused_reorder_by_indicies() for _ in range(5)]))
