...
class PartitionVariableDef(object):
  ...
  @property
  def dtype(self):
    return [tf.dtypes.DType(node.attr['dtype'].type)
            for node in self._partitions.values()]

  @property
  def shape(self):
    return [tuple(dim.size for dim in node.attr['shape'].shape.dim)
            for node in self._partitions.values()]
  ...
  def initializer(self):
    if self._initializer is None:
      dest_nodes = [_node_name(f'{pname}/PartitionedInitializer/Slice')
          ...
          with tf.compat.v1.device(None):
            with tf.compat.v1.device(group_device):
              variable = tf.compat.v1.get_variable(
                dtype=dtypes[i], shape=shapes[i],
                initializer=inits[i], name=name)
...
class GraphDefHelper(object):
  def __init__(self, graph_def: tf.compat.v1.GraphDef):
    ...
    self.name_to_vardef: Dict[str, Union[VariableDef, PartitionVariableDef]] = {}
    self.name_to_input_name: Dict[str, Set[str]] = {}  # Keyed by the dest node name.
  ...
  @classmethod
  def _check_invalidate_node(clz, 
        ...
        for pname, v in part_var.items():
          ...
          for reader in vardef.read_ops(pname):
            if reader.name == f'{reader.input[0]}/Read/ReadVariableOp':
              continue
            vread_map[reader.name] = read_variable_op(
              resource=v._handle, dtype=v.dtype, name=_node_name(reader.name))
      else:
        # remove variable that outside the graph
  ...
  def sub_graph(self,
    ...
    nodes_to_keep_list = sorted(
      list(nodes_to_keep), key=lambda name: self.name_to_seq_num[name])
    ...
    if with_library:
      for func in self.graph_def.library.function:
        if "Dataset" in func.signature.name:
  ...
  def import_input_fn(self, input_conf, file_name: str):
    ...
    sub_graph, _ = self.sub_graph(dest_nodes=dest_nodes, source_nodes=None, with_library=True)
    ...
    result = {}
    for i, (feat_name, ts_repr) in enumerate(input_conf.output_features.items()):
      ts_dict = eval(ts_repr)
      if ts_dict['is_ragged']:
        row_splits, values = return_elements[i].outputs
        assert ts_dict['name'] == values.name.split(':')[0]
        row_partition = RowPartition.from_row_splits(row_splits=tf.reshape(row_splits, shape=(-1,)), 
                                                     validate=False, preferred_dtype=None)
        result[feat_name] = tf.RaggedTensor(tf.reshape(values, shape=(-1,)), row_partition, internal=True)
      else:
  ...
  def import_model_fn(self, input_map: Dict[str, tf.Tensor], proto_model):
    ...
    result = tf.import_graph_def(sub_graph,
                                 input_map=vread_map if vread_map else None,
                                 return_elements=outputs,
                                 name="")
    if model_fn.label is not None and len(model_fn.label) > 0:
      idx = outputs.index(model_fn.label)
      label = result[idx]
    else:
      label = None
    ...
    if model_fn.head_name is None or len(model_fn.head_name) == 0:
      head_name = None
    else:
      head_name = model_fn.head_name.split(',')
      if len(head_name) == 1:
    ...
    if len(proto_model.extra_output) > 0:
      for extra_output in proto_model.extra_output:
        ...
        if len(extra_output.fetch_dict) == 1 and key == result[idx].name:
          extra_output_dict[extra_output.signature_name] =  next(iter(real_extra.values()))
        else:
          extra_output_dict[extra_output.signature_name] = real_extra
    
    return label, loss, predict, head_name, extra_output_dict
  ...
  def import_receiver_fn(self, receiver_conf):
    ...
    for feat_name, ts_repr in receiver_conf.features.items():
      ...
      if ts_dict['is_ragged']:
        ...
        features[feat_name] = tf.RaggedTensor.from_row_splits(values, row_splits, validate=False)
        idx += 2
      else:
