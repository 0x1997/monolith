...
from monolith.native_training.model_dump.graph_utils import DatasetInitHook, GraphDefHelper, DRY_RUN
...
class DumpUtils(object):
      ...
      if self.need_record:
          ...
          for p, value in zip(params, args):
            ...
            try:
              if p.name == 'feature_name' and isinstance(value, int):
                setattr(proto, p.name, get_slot_feature_name(value))
              else:
        ...
        for key, value in kwargs.items():
          try:
            ...
            logging.warning(f"{key} is not in proto, {e}")
  ...
  def record_params(self, model):
    ...
    skip_attrs = {'_abc_impl', '_ctx', '_export_outputs', '_global_step', '_losses',
                  '_occurrence_threshold', '_private_children',
                  'children', 'ctx', 'fc_dict', 'fs_dict', 'losses', 'slice_dict',
                  '_layout_dict'}
  ...
  def get_params_bytes(self, model) -> bytes:
    ...
    params = {'layout_dict'} | self._params
    model_params = {}
    for attr_name in params:
      ...
      elif attr_name == 'layout_dict':
        if attr:
          model_params[attr_name] = {name: out_cfg.SerializeToString()
  ...
  def add_model_fn(self,
                   ...
                   pred: Union[tf.Tensor, List[tf.Tensor]],
    ...
    if label is not None:
      model_fn.label = label.name
    if loss is not None:
      model_fn.loss = loss.name
    if isinstance(pred, (tuple, list)):
      model_fn.predict.extend([t.name for t in pred])
    else:
      model_fn.predict.append(pred.name)

    if head_name is not None:
      if isinstance(head_name, str):
        model_fn.head_name = head_name
      else:
        assert isinstance(head_name, (list, tuple))
        model_fn.head_name = ','.join(head_name)
    ...
    if training_hooks:
      if len(training_hooks) == 1 and isinstance(training_hooks[0], ItemPoolSaveRestoreHook):
        pass
      else:
  ...
  def add_input_fn(self, results: Dict[str, Union[tf.Tensor, tf.RaggedTensor]]):
    ...
    if isinstance(results, (list, tuple)):
      features, label = result[0], result[1]
    else:
  ...
  def get_proto_model(self, mode: str = tf.estimator.ModeKeys.TRAIN) -> ProtoModel:
    graph = tf.compat.v1.get_default_graph()
    if mode == tf.estimator.ModeKeys.TRAIN:
      return self.train
    else:
  ...
  def get_graph_helper(self, mode: str) -> GraphDefHelper:
    ...
    if hasattr(graph, 'graph_def_helper'):
      ...
      if mode == tf.estimator.ModeKeys.TRAIN:
        helper = GraphDefHelper(self.train_graph)
      else:
        helper = GraphDefHelper(self.infer_graph)
  ...
  def restore_params(self) -> Dict[str, Any]:
    ...
    layout_dict = model_params.get('layout_dict')
    if layout_dict:
      layout_dict_tmp = {}
  ...
  def table_configs(self) -> Dict[str, entry.HashTableConfigInstance]:
    ...
    for tcfg in self._table_configs:
      ...
      result[tcfg.name] = entry.HashTableConfigInstance(
        table_config, list(tcfg.learning_rates), list(tcfg.extra_restore_names))
    return result
  
  @table_configs.setter
  def table_configs(self, table_confs: Dict[str, entry.HashTableConfigInstance]):
    if table_confs:
      ...
      for name, tcfg in table_confs.items():
        hash_table_config = HashTableConfig(
          name=name, 
          table_config=tcfg._table_config.SerializeToString()
        )
  ...
  def feature_combiners(self, fcombs):
    if fcombs:
      ...
      for name, fcomb in fcombs.items():
        fc_proto = FeatureCombiner(name=name, max_seq_length=fcomb.max_seq_length)
  ...
  def has_collected(self) -> bool:
    if self._table_configs and self._feature_slice_dims and self._feature_combiners:
      ...
      assert self._feature_slice_dims is None or len(self._feature_slice_dims) == 0
      assert self._feature_combiners is None or len(self._feature_combiners) == 0
      return False
