...
class DataOpsTest(tf.test.TestCase):
  ...
  def test_string_to_variant(self):
    ...
    for i in range(10):
      ...
      if i == 3:
        inst_str = b''
      if has_header:
        data = struct.pack(f'<{len(header)}sQ{len(inst_str)}s', header, len(inst_str), inst_str)
      else:
        data = inst_str
      insts.append(data)
    with tf.Graph().as_default():
      ...
      ops = string_to_variant(ips, variant_type='instance', has_header=has_header, lagrangex_header=lg_header_flag,
                              has_sort_id=sort_id, kafka_dump=kafka_dump, kafka_dump_prefix=kafka_dump_prefix)
  ...
  def test_has_variant(self):
    ...
    with tf.Graph().as_default():
      ...
      ops = string_to_variant(ips, variant_type='instance', has_header=True, lagrangex_header=False,
                              has_sort_id=False, kafka_dump=False, kafka_dump_prefix=False)
