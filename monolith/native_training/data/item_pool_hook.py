...
class ItemPoolSaveRestoreHook(SessionRunHook):
  ...
  def begin(self): 
    ...
    if pools:
      ...
      logging.info("get the ckpt_state from model_dir: {}".format(self._model_dir))
      ...
      if self._ckpt_state:
        ...
        self._restore_op = restore_item_pool(pool=pools[0],
  ...
  def after_create_session(self, session, coord):  # pylint: disable=unused-argument
    ...
    if self._restore_op is not None:
      logging.info("the last global step is {}".format(str(self._last_global_step)))
      if self._ckpt_state:
        step = int(self._ckpt_state.model_checkpoint_path.split('/')[-1].split('-')[-1])
        logging.info("the step from the last checkpoint is {}".format(str(step)))
        session.run(self._restore_op, feed_dict={self._restore_global_step:step})
        logging.info("after_create_session retore the itempool from ckpt: {}".format(str(step)))
  ...
  def after_run(self,
    ...
    if self._save_op is not None and self._save_steps is not None and self._save_steps > 0:
      ...
      if cur_global_step > self._last_global_step + self._save_steps:
        logging.info("after_run start to save item_pool at step {}".format(str(cur_global_step)))
        run_context.session.run(self._save_op, feed_dict={self._save_global_step: cur_global_step})
  ...
  def end(self, session):  # pylint: disable=unused-argument
    ...
    if self._save_op is not None:
      ...
      if cur_global_step > self._last_global_step:
        logging.info("session_end start to save item_pool at step {}".format(str(cur_global_step)))
        session.run(self._save_op, feed_dict={self._save_global_step : cur_global_step})
        self._last_global_step = cur_global_step
