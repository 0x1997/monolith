...
class ItemPoolTest(tf.test.TestCase):
  ...
  def setUpClass(cls):
    ...
    pool = create_item_pool(start_num=20, max_item_num_per_channel=100, shared_name='first')
    pool = item_pool_random_fill(pool)
    pool = save_item_pool(pool, model_path=cls.model_path, global_step=global_step, nshards=2)
  
  def test_create_item_pool(self):
    global_step = tf.constant(1, dtype=tf.int64)
    pool = create_item_pool(start_num=20, max_item_num_per_channel=100, shared_name='second')
    pool = restore_item_pool(pool, model_path=self.model_path, global_step=global_step, nshards=2)
    pool = item_pool_check(pool, global_step=global_step, model_path=self.model_path, nshards=2)
