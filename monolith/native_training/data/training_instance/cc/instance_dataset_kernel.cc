...
using ::tensorflow::monolith_tf::ReaderOptions;
    ...
    class Iterator : public DatasetIterator<Dataset> {
     public:
      ...
      Status GetNextInternal(IteratorContext* ctx,
        ...
        Status s =
            reader_->next(&offset_, &data_source_key, &out_tensors->back().scalar<tstring>()());
        if (s.ok()) {
     ...
     private:
      ...
      Status SetupStreamsLocked(Env* env) TF_EXCLUSIVE_LOCKS_REQUIRED(mu_) {
        if (dataset()->file_name_.empty()) {
          reader_ = absl::make_unique<PBIterator>(PRUNING_RAW_FEATURE,
                                                  dataset()->options_);
        } else {
          reader_ = absl::make_unique<PBIterator>(env, dataset()->file_name_,
                                                  PRUNING_RAW_FEATURE,
                                                  dataset()->options_);
        }
        return Status::OK();
