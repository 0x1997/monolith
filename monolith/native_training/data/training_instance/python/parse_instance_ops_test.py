...
def generate_instance():
  ...
  instance.line_id.external_action = "test_action"
  return instance
...
class RaggedEncodingHelperTest(tf.test.TestCase):
  ...
  def testExpandContract(self):
    ...
    ops.RaggedEncodingHelper.expand(d)
    assert len(d) == 2
    self.assertAllEqual(
        d["slot_2" + ops.RaggedEncodingHelper.suffix_value_rowids],
        rt_copy.value_rowids())
    ops.RaggedEncodingHelper.contract(d)
    assert len(d) == 1
    self.assertAllEqual(rt._row_partition._value_rowids, rt_copy.value_rowids())
  ...
  def testParseInstance(self):
    ...
    with tf.compat.v1.Session() as sess:
      features = ops.parse_instances2(
          ...
          misc_int64_dims=[1, 2],
          misc_string_features=["external_action"],
          misc_string_dims=[1])
      ...
      self.assertAllEqual(features["external_action"], [["test_action"]] * 2)
