 ...
 public:
  InnerIterator(IteratorBase *input_impl, ItemPoolResource *resource,
    ...
    for (size_t i = 0; i < action_priority_items.size(); ++i) {
      int32 action;
      assert(absl::SimpleAtoi(action_priority_items[i], &action));
    ...
    if (variant_type == "instance") {
      ...
      if (index_feature_.empty()) {
        ...
        assert(absl::SimpleAtoi(index_feature_, &index_slot_));
      }

      if (channel_feature_.empty()) {
        channel_slot_ = 3;
      } else {
        assert(absl::SimpleAtoi(channel_feature_, &channel_slot_));
      }

      for (const auto &fname : item_features_) {
        int32 slot;
        assert(absl::SimpleAtoi(fname, &slot));
...
class InstanceNegativeGenDatasetOp::Dataset : public DatasetBase {
 public:
  Dataset(OpKernelContext *ctx, const DatasetBase *input, int32 neg_num,
          ...
          float neg_sample_declay_factor, const std::string &variant_type,
   ...
   public:
    ...
    Status Initialize(IteratorContext *ctx) override {
      ...
      iter_ = std::make_unique<InnerIterator>(
          ...
          dataset()->neg_sample_declay_factor_, dataset()->variant_type_);
      return s;
    }
...
void InstanceNegativeGenDatasetOp::MakeDataset(OpKernelContext *ctx,
  ...
  *output = new Dataset(
      ...
      origin_neg_in_pool_proba_, neg_sample_declay_factor_, variant_type_,
