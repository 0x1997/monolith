...
class _CpuFeatureFactory(feature.FeatureFactoryFromEmbeddings):
  ...
  def __init__(self,
    ...
    super().__init__(embeddings, embedding_ids, feature_to_combiner,
...
class _FusedCpuFeatureFactory(feature.FeatureFactoryFromEmbeddings):
  ...
  def __init__(self, hash_table: multi_type_hash_table.BaseMultiTypeHashTable,
    ...
    self._embedding_ids = name_to_embedding_ids
    self._req_time = req_time
    # Pass in ragged tensor and ID/embedding mapping, to fuse split/reduce_op.
    super().__init__(self._embeddings, name_to_embedding_ids,
  ...
  def apply_gradients(self,
    ...
    apply_op = self._hash_table.apply_gradients(slot_to_ids_and_grads,
...
class CpuTrainingConfig:
  ...
  dense_only_stop_training_when_save: bool = None
...
def make_native_task_context(config: CpuTrainingConfig,
  ...
  return native_task_context.NativeTaskContext(num_ps=config.num_ps,
                                               num_workers=config.num_workers,
                                               worker_index=config.index,
...
class CpuTraining:
  ...
  def __init__(self,
    ...
    if not config.model_name:
      config.model_name = task.p.metrics.deep_insight_name
    if config.dense_only_stop_training_when_save is None:
      # Maybe we should make it False by default.
      config.dense_only_stop_training_when_save = True
    if config.partial_recovery is None:
      config.partial_recovery = config.enable_realtime_training
    ...
    with native_task_context.with_ctx(
      ...
      if hasattr(self._task, 'is_dummed') or dump_utils.enable:
        if dump_utils.has_collected:
          feature_name_config: Dict[str, entry.HashTableConfigInstance] = dump_utils.table_configs
          feature_to_unmerged_slice_dims: Dict[str, List[int]] = dump_utils.feature_slice_dims
          feature_to_combiner = dump_utils.feature_combiners  # Dict[str, embedding_combiners.Combiner]
          self._feature_configs_do_not_refer_directly = (feature_name_config,
          ...
          dump_utils.table_configs = self._feature_configs_do_not_refer_directly[0]
          dump_utils.feature_slice_dims = self._feature_configs_do_not_refer_directly[1]
          dump_utils.feature_combiners = self._feature_configs_do_not_refer_directly[2]
      else:
        self._feature_configs_do_not_refer_directly = self._collect_feature_name_to_table_config(
        )
  ...
  def create_input_fn(self, mode):
    ...
    def _create_parse_fn():
      ...
      def wrapped_parse_fn(*args):
        ...
        # Results include the following intermediate tensors to avoid duplicate computation.
        res = (
            features,
            # CpuTraining.create_model_fn: def model_fn
            (
                parser_utils.RaggedEncodingHelper.expand(embedding_ragged_ids),
                name_to_embedding_ids,
  ...
  def create_model_fn(self):
    ...
    def create_hash_table_and_filters_fn():
      ...
      if not self.config.enable_sync_training:
        ...
        if self.config.enable_fused_layout:
          return distributed_ps_factory.create_partitioned_hash_table(
            num_ps=self.config.num_ps, feature_name_to_config=feature_name_config,
            layout_configs=self._task.layout_dict,
            feature_to_combiner=feature_to_combiner,
            feature_to_unmerged_slice_dims=feature_to_unmerged_slice_dims,
            use_native_multi_hash_table=self.config.use_native_multi_hash_table,
            max_rpc_deadline_millis=self.config.max_rpc_deadline_millis,
            unique=unique, transfer_float16=False,
            hash_filters=hash_filters, sync_clients=sync_clients
          ), hash_filters
        elif self.config.use_native_multi_hash_table:
  ...
  def _get_pipelined_model_fn(self, create_hash_table_and_filters_fn: Callable[
    ...
    def get_hooks_for_metrics(model_dir: str, save_steps: int):
      ...
      if self._params.metrics.enable_throughput_hook and self._params.metrics.deep_insight_name and is_chief(
          self.config):
        hooks.append(
    ...
    def get_partitioner_variable_context():
      if self.config.enable_variable_partition:
    ...
    def model_fn(features: Dict[str, tf.Tensor], mode: str,
      ...
      if self.config.enable_fused_layout:
        ...
        logging.info(
            f"hash_table lookup when enable_fused_layout: {layout_embeddings} --- {auxiliary_bundle}"
        )
        # embedding_prefetch
        ...
        self._task.ctx.layout_factory = feature.EmbeddingLayoutFactory(
            hash_table,
            deq_layout_embeddings,
            auxiliary_bundle=deq_auxiliary_bundle)
        ...
        if self.config.reorder_fids_in_data_pipeline:
          ...
          embedding_ragged_ids, name_to_ids, res_pack = res_pack
          parser_utils.RaggedEncodingHelper.contract(embedding_ragged_ids)
        ...
        self._task.ctx.layout_factory = None
        dequeued_features.update(dequeued_embeddings)
        if self.config.enable_sync_training:
          self._task.ctx.feature_factory = _FusedCpuFeatureFactory(
              ...
              feature_to_combiner,
              feature_to_unmerged_slice_dims,
              get_req_time(dequeued_features),
      ...
      with get_cached_variable_context(), get_partitioner_variable_context():
        spec = raw_model_fn(features=dequeued_features,
                            mode=mode,
                            config=config)
...
class DistributedCpuTrainingConfig(CpuTrainingConfig):
  ...
  operation_timeout_in_ms: int = 600000
...
def distributed_sync_train(config: DistributedCpuTrainingConfig,
  ...
  if config.enable_gpu_training:
    ...
    if os.environ.get('MONOLITH_FORCE_GPU_COMPATIBLE', '0') == '1':
    ...
    if enable_bps and (os.environ.get('MONOLITH_WITH_BYTEPS_FWD_GDR', '0') == '1' or \
      ...
      session_config.gpu_options.visible_device_list = device_utils.get_visible_gpus(
          hvd.local_rank(), processes_per_gpu=config.processes_per_gpu)
  ...
  root_interval = int(os.environ.get('MONOLITH_ROOT_LOG_INTERVAL', '100'))
  run_config = tf.estimator.RunConfig(
      model_dir=config.model_dir,
      device_fn=device_utils.default_device_fn,
      session_config=session_config,
      save_summary_steps=None if config.index != 0 else 10 * root_interval,
      log_step_count_steps=params.train.max_steps
      if config.index != 0 else root_interval)
  ...
  run_hooks.append(sync_training_hooks.SyncTrainingRefreshTokenHook())
