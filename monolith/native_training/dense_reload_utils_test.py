...
class DenseReloadUtilsTest(tf.test.TestCase):
  ...
  def setUpClass(cls):
    with tf.Graph().as_default():
      ...
      partitioner = tf.compat.v1.variable_axis_size_partitioner(max_shard_bytes=1 << 17, max_shards=100)
      partition_var = tf.compat.v1.get_variable(name='partition', shape=(1280, 512), dtype=tf.float32,
                                                partitioner=partitioner, initializer=GlorotNormal())

      var = tf.compat.v1.get_variable(name='small_var', shape=(10, 5), dtype=tf.float32,
                                      initializer=Ones())
      ...
      with tf.compat.v1.Session() as sses:
        sses.run(init)
        saver.save(sses, save_path=f"{os.getcwd()}/ckpt/test", global_step=global_step)
  ...
  def test_infer_variable_name(self):
    with tf.Graph().as_default():
      ...
      self.assertSetEqual(infer_variable_name(names), {f'{partition_var.name}:0'})

  def test_calc_feed_dict(self):
    with tf.Graph().as_default():
      global_step = tf.compat.v1.train.get_or_create_global_step()
      partitioner = tf.compat.v1.variable_axis_size_partitioner(max_shard_bytes=1 << 17, max_shards=100)
      partition_var = tf.compat.v1.get_variable(name='partition2', shape=(1280, 512), dtype=tf.float32,
                                                partitioner=partitioner, initializer=GlorotNormal())

      var = tf.compat.v1.get_variable(name='small_var2', shape=(10, 5), dtype=tf.float32,
                                      initializer=Ones())
      ...
      ph_dict = calc_feed_dict(ckpt, alias_map=alias_map, placeholders=placeholders)
  ...
  def test_alias_map_listener(self):
    with tf.Graph().as_default():
      ...
      listener = CustomRestoreListener(alias_map=alias_map, model_dir=f"{os.getcwd()}/ckpt")
      listener.begin()
  ...
  def test_clear_nn_listener(self):
    with tf.Graph().as_default():
      ...
      var = tf.compat.v1.get_variable(name='small_var2', shape=(10, 5), dtype=tf.float32,
                                      initializer=Ones())
      listener = CustomRestoreListener(clear_nn=True, model_dir=f"{os.getcwd()}/ckpt")
      listener.begin()
