...
#include "monolith/native_training/runtime/neo_filter/hash_filter.h"

template <typename DATA>
void test_simple_bloom(size_t key_num) {
  neo::HashFilter<DATA> counter(key_num);
  for (uint32_t i = 0; i <= neo::HashFilter<DATA>::max_count(); ++i) {
    uint32_t expect = std::min(i + 1, neo::HashFilter<DATA>::max_count());
    EXPECT_EQ(i, counter.add(1, 1));
    EXPECT_EQ(expect, counter.get(1));
  }
  ASSERT_EQ(neo::HashFilter<DATA>::max_count(), counter.add(1, 1));
  ASSERT_EQ(neo::HashFilter<DATA>::max_count(), counter.get(1));
}
...
template <typename DATA>
void test_count() {
  neo::HashFilter<DATA> filter(1000000);
...
void compare_to_unordered_map(int key_number, double expected_rate,
                              double fill_rate) {
  neo::HashFilter<DATA> filter(key_number, fill_rate);
  std::unordered_map<int, uint32_t> map_counter;
  std::srand(std::time(NULL));
  for (int i = 0; i != key_number; ++i) {
    int num = std::rand();
    if (map_counter[num] < neo::HashFilter<DATA>::max_count() - 1) {
      map_counter[num] += 2;
      filter.add(num, 2);
    }
  }
...
void TestSkipZeroThresholdFeatures() {
  neo::HashFilter<DATA> filter(1000000, 10);
