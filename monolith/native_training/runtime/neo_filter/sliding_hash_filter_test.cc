...
#include "monolith/native_training/runtime/neo_filter/types.h"

static void test_simple_bloom(size_t key_num) {
  neo::SlidingHashFilter counter(key_num, 10);
  for (uint32_t i = 0; i <= neo::HashFilter<uint16_t>::max_count(); ++i) {
    EXPECT_EQ(i, counter.add(1, 1));
  }
  ASSERT_EQ(neo::HashFilter<uint16_t>::max_count(), counter.add(1, 1));
}
...
TEST(SlidingHashFilterTest, test_count) {
  neo::SlidingHashFilter filter(1000000, 10);
  ...
  neo::SlidingHashFilter filter2(1000000, 10);
  filter2.add(10000002961562801052lu, 1);
  filter2.add(10000002961562801052lu, 20);
  filter2.add(10000002961562801052lu, 1);
  EXPECT_EQ(1llu, filter2.estimated_total_element());

  std::unique_ptr<neo::SlidingHashFilter> filter3(filter2.clone());
  EXPECT_TRUE(filter2 == *filter3);
}

template <typename Container>
static void check_conflict_rate(neo::SlidingHashFilter& filter,
...
static void compare_to_unordered_map(int key_number, double expected_rate,
  ...
  std::srand(capacity);
  int split_num = 10;
  neo::SlidingHashFilter filter(capacity, split_num);
  std::unordered_map<int, uint32_t> map_counter;
  for (int i = 0; i != key_number; ++i) {
    int num = std::rand();
    if (map_counter[num] < neo::HashFilter<uint16_t>::max_count() - 1) {
      map_counter[num] += 2;
      filter.add(num, 2);
    }
  }
