...
class HashTableFusedOptimizeOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    const Tensor& id_offsets = ctx->input(num_tables_ + 3);
    const auto& id_offsets_flat = id_offsets.vec<int32>();
    const Tensor& grad_offsets = ctx->input(num_tables_ + 4);
    const auto& grad_offsets_flat = grad_offsets.vec<int32>();
    ...
    const Tensor& learning_rate_lengths = ctx->input(num_tables_ + 6);
    const auto& learning_rate_lengths_flat = learning_rate_lengths.vec<int32>();
    const Tensor& update_time_tensor = ctx->input(num_tables_ + 7);
    int64_t update_time = update_time_tensor.scalar<int64_t>()();
    const Tensor& global_step = ctx->input(num_tables_ + 8);
    int64_t global_step_value = global_step.scalar<int64_t>()();
