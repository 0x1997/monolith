...
NeoFilterTfBridge::NeoFilterTfBridge(
    std::unique_ptr<neo::Filter> filter,
    const SlotOccurrenceThresholdConfig& config)
...
int NeoFilterTfBridge::GetSlotOccurrenceThreshold(int64_t fid) const {
  return slot_to_occurrence_threshold_[slot_id_v2(fid)];
}

Status NeoFilterTfBridge::Save(
...
Status NeoFilterTfBridge::Restore(
