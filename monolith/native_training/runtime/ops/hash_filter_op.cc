...
#include "monolith/native_training/runtime/neo_filter/dummy_hash_filter.h"
#include "monolith/native_training/runtime/neo_filter/probabilistic_filter.h"
#include "monolith/native_training/runtime/neo_filter/sliding_hash_filter.h"
#include "monolith/native_training/runtime/ops/neo_filter_tf_bridge.h"
...
class DummyFilterOp : public ResourceOpKernel<NeoFilterTfBridge> {
 public:
 ...
 private:
  Status CreateResource(NeoFilterTfBridge** filter_bridge)
      TF_EXCLUSIVE_LOCKS_REQUIRED(mu_) override {
    auto filter = std::make_unique<neo::DummyHashFilter>();
    *filter_bridge = new NeoFilterTfBridge(std::move(filter), config_);
    return Status::OK();
  };
...
class HashFilterOp : public ResourceOpKernel<NeoFilterTfBridge> {
 public:
 ...
 private:
  Status CreateResource(NeoFilterTfBridge** filter_bridge)
      TF_EXCLUSIVE_LOCKS_REQUIRED(mu_) override {
    auto filter =
        std::make_unique<neo::SlidingHashFilter>(capacity_, split_num_);
...
class ProbabilisticFilterOp : public ResourceOpKernel<NeoFilterTfBridge> {
 public:
 ...
 private:
  Status CreateResource(NeoFilterTfBridge** filter_bridge)
      TF_EXCLUSIVE_LOCKS_REQUIRED(mu_) override {
    auto filter =
        std::make_unique<neo::ProbabilisticFilter>(equal_probability_);
    *filter_bridge = new NeoFilterTfBridge(std::move(filter), config_);
    return Status::OK();
  };
