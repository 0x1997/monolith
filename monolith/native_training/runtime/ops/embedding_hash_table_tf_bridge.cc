...
Status EmbeddingHashTableTfBridge::New(
    ...
    NeoFilterTfBridge* hash_filter, EmbeddingHashTableTfBridge** new_bridge) {
  ...
  try {
    bridge->table_ = hash_table::NewEmbeddingHashTableFromConfig(config);
...
Status EmbeddingHashTableTfBridge::BatchLookup(OpKernelContext* ctx,
  ...
  try {
    ...
    table_->BatchLookup(absl::MakeSpan(ids, num_ids),
