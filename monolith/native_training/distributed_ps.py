...
class PartitionedHashTable(object):
  ...
  @classmethod
  def no_merge_feature_config(cls, feature_name_to_config: Dict[str, entry.HashTableConfigInstance]):
  ...
  @classmethod
  def merge_feature_config(cls, feature_name_to_config: Dict[str, entry.HashTableConfigInstance]):
  ...
  def __init__(self,
    ...
    if self._use_native_multi_hash_table:
      ...
      self._sub_table_name_to_config, feature_to_sub_table = self.merge_feature_config(feature_name_to_config)

    feature_info = {}
    for feature_name, sub_table in feature_to_sub_table.items():
      feature_info[feature_name] = FeatureInfo(feature_to_unmerged_slice_dims[feature_name],
      feature_to_combiner[feature_name], sub_table)
    self._feature_configs: FeatureConfigs = self.gen_feature_configs(feature_info, layout_configs)

    sub_table_to_learning_rate_tensor = {sub_table_name: config.call_learning_rate_fns()
  ...
  def _native_hash_table_lookup_raw(self, lookup_data_on_wk: LookupData,
    ...
    for i in range(self._num_ps):
      ...
      splitted_id = tf.RaggedTensor.from_row_splits(
          ...
          validate=False)
      is_standalone = export_context.is_exporting_standalone(
        ) or self._local_ps
  ...
  def _lookup_raw(self, lookup_data_on_wk: LookupData):
    ...
    for i in range(self._num_ps):
      ...
      if self.transfer_float16:
        ...
        packed_embeddings_fp32 = tf.cast(
            packed_embeddings_fp16_recv,
            dtype=tf.float32,
            name='{}_recv_{}_CastToFloat32'.format(
  ...
  def _native_hash_table_lookup_with_remote_predict(
    ...
    for i in range(self._num_ps):
      flat_emb, = remote_predict(
          ...
          output_types=[tf.float32],
          signature_name="raw_lookup")
      ps_idx_to_multi_type_resp[i] = {self._native_multi_hash_table_fake_table: flat_emb}
    return ps_idx_to_multi_type_resp
    ...
    with tf.name_scope("pht_lookup"):
      ...
      if export_context.is_exporting_distributed():
        if self._use_native_multi_hash_table:
          ...
          ps_idx_to_multi_type_resp = self._lookup_with_remote_predict(lookup_data_on_wk)
      else:
        if self._use_native_multi_hash_table:
  ...
  def _native_hash_table_update(self, method_name: str, name_scope: str,
