...
class FIFOQueueTest(tf.test.TestCase):
  ...
  def test_fifo_queue_data(self):
    ...
    nested = prefetch_queue._NestedTensors((dense_tensors, ragged_tensors))
...
class PrefetchTest(tf.test.TestCase):
  ...
  def test_enqueue_dicts_with_queue_return(self):
    ...
    result = prefetch_queue.enqueue_dicts_with_queue_return((dense_dicts,ragged_dicts), capacity=3)
    ...
    with self.session() as sess:
      for _ in range(5):
        ...
        sess.run(queue.enqueue_op)
        dense_dicts_result, ragged_dicts_result = sess.run([dequeue_dense_dicts, dequeue_ragged_dicts])
  ...
  def test_enqueue_dicts_with_control_flow(self):
    ...
    with tf.control_dependencies([v.assign_add(1)]):
      tensor, q = prefetch_queue.enqueue_dicts_with_queue_return(
          tf.constant(0))
  ...
  def test_estimator_prefetch(self):
    ...
    def model_fn(features, mode):
      ...
      dequeue_raggeds, queue = prefetch_queue.enqueue_dicts_with_queue_return(ragged_dicts)
