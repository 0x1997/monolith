...
class _NestedTensors:
  ...
  def get_tensors(self) -> Tuple[List[tf.Tensor], List[tf.RaggedTensor]]:
  ...
  def _ragged_to_flatten(self, ragged_tensors):
    return list(
        chain.from_iterable((self._convert_ragged_to_tensors(ragged)
