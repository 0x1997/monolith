...
class TFSMonitor(object):
  def __init__(self, config: AgentConfig):
    ...
    self._host = None
    self._lock = RLock()
    self.stubs = {TFSServerType.ENTRY: {}, TFSServerType.PS: {}}
  ...
  def get_addr(self, sub_model_name: SubModelName) -> str:
    if self._conf.deploy_type == DeployType.MIXED:
      ...
      assert not self.is_entry(sub_model_name)
      return f"{self.host}:{self._conf.tfs_ps_port}"
    else:
  ...
  def get_service_type(self, sub_model_name: SubModelName):
    if self._conf.deploy_type == DeployType.ENTRY:
      return TFSServerType.ENTRY if sub_model_name.startswith(TFSServerType.ENTRY) else None
    elif self._conf.deploy_type == DeployType.PS:
      return TFSServerType.PS if sub_model_name.startswith(TFSServerType.PS) else None
    else:
  ...
  def connect(self):
    if self._conf.deploy_type in {DeployType.MIXED, DeployType.ENTRY}:
      entry_channel = grpc.insecure_channel(f'{self.host}:{self._conf.tfs_entry_port}')
      self.stubs[TFSServerType.ENTRY]['channel'] = entry_channel
      self.stubs[TFSServerType.ENTRY]['model_service'] = ModelServiceStub(entry_channel)
      self.stubs[TFSServerType.ENTRY]['prediction_service'] = PredictionServiceStub(
        entry_channel)

    if self._conf.deploy_type in {DeployType.MIXED, DeployType.PS}:
      ps_channel = grpc.insecure_channel(f'{self.host}:{self._conf.tfs_ps_port}')
      self.stubs[TFSServerType.PS]['channel'] = ps_channel
      self.stubs[TFSServerType.PS]['model_service'] = ModelServiceStub(ps_channel)
      self.stubs[TFSServerType.PS]['prediction_service'] = PredictionServiceStub(
        ps_channel)
  ...
  def _(self, pm: PublishMeta, fix_dense_version: bool = False):
    ...
    with self._lock:
      ...
      for sub_model_name, smvpath in pm.sub_models.items():
        ...
        if self.is_entry(sub_model_name) and not fix_dense_version:
        ...
        try:
          model_version_status = stub.GetModelStatus(request).model_version_status
          if model_version_status is None or len(model_version_status) == 0:
            ...
            model_version_status = sorted(model_version_status, key=lambda mvs: mvs.version)
            status = model_version_status[-1]
        except grpc._channel._InactiveRpcError as e:
          logging.info(repr(e))
          status = ModelVersionStatus(state=State.UNKNOWN,
                                      status=StatusProto(error_code=e.code().value[0],
  ...
  def _(self,
    ...
    with self._lock:
      ...
      if service_type is None:
        ...
        request = GetModelStatusRequest()
        request.model_spec.CopyFrom(gen_model_spec(name, version, signature_name))
        stub = self.stubs[service_type]['model_service']
        return stub.GetModelStatus(request).model_version_status

  def gen_model_config(self, pms: Iterable[PublishMeta],
    ...
    for pm in pms:
      ...
      for sub_model_name, smv_path in pm.sub_models.items():
        ...
        if self.is_entry(sub_model_name):
          ...
          version_data = int(os.path.basename(smv_path)) if fix_dense_version else 1
        else:
          version_policy = 'specific'
          version_data = int(os.path.basename(smv_path))

        model_config = gen_model_config(tfs_model_name, base_path, version_policy, version_data)
        model_configs[service_type].model_config_list.config.append(model_config)

    return model_configs

  def handle_reload_config_request(self,
                                   service_type: str,
                                   model_configs: ModelServerConfig) -> StatusProto:
    with self._lock:
      ...
      port = self._conf.tfs_entry_port if service_type == TFSServerType.ENTRY else self._conf.tfs_ps_port
      logging.info(f'{service_type} load ({port}): \n{request}')
      try:
        response = self.stubs[service_type]['model_service'].HandleReloadConfigRequest(request)
