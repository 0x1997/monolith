...
class ModelManagerTest(unittest.TestCase):
    def create_file(self, model_name, timestamp, p2p_data_path):
        ...
        os.makedirs(os.path.join(p2p_data_path, model_name + '@' + timestamp, model_name, 'ps_item_embedding_0', timestamp))
        os.makedirs(os.path.join(p2p_data_path, model_name + '@' + timestamp, model_name, 'ps_item_embedding_1', timestamp))

        f = open(os.path.join(p2p_data_path, model_name + '@' + timestamp + '.write.done'), 'w+')
        f.close()
    ...
    def test_start(self):
        ...
        model_manager = ModelManager(model_name, p2p_data_path, model_data_path, False)
        model_manager._wait_timeout = 5
        model_manager._loop_interval = 5
        ret = model_manager.start()
        self.assertTrue(ret)

        ready_path1 = os.path.join(model_data_path, model_name, 'ps_item_embedding_0', timestamp)
        ready_path2 = os.path.join(model_data_path, model_name, 'ps_item_embedding_1', timestamp)
    ...
    def test_ignore_old(self):
        ...
        ready_path1 = os.path.join(model_data_path, model_name, 'ps_item_embedding_0', timestamp_old)
        ready_path2 = os.path.join(model_data_path, model_name, 'ps_item_embedding_1', timestamp_old)
