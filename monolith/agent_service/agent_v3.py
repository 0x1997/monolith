...
export TCE_PSM="data.monolith.yijie_zhu"
export TCE_CLUSTER="default"
export TCE_INTERNAL_IDC="LF"
export MONOLITH_TFS_BINARY="/home/yijie.zhu/monolith/bazel-bin/external/org_tensorflow_serving/tensorflow_serving/model_servers/tensorflow_model_server"

2) start agent locally
bazel run monolith/agent_service:agent -- --conf=/home/yijie.zhu/monolith/monolith/agent_service/agent_v3_demo.conf \
  --tfs_log=/home/yijie.zhu/tmp/tfs.std.log

3) see agent_controller.py
"""
...
class AgentV3(AgentBase):
  ...
  def __init__(self, config: AgentConfig, conf_path:str, tfs_log: str):
    ...
    self._tfs_wrapper = TFSWrapper(
      config.tfs_port_archon, 
      config.tfs_port_grpc, 
    ...
    self._container = Container(self.config.container_cluster, self.config.container_id)
    ...
    self._service_info = ContainerServiceInfo(
      ...
      debug_info=json.dumps({
        ...
        'layout_filters': [f"{match};{cond}" for match, cond in self._layout_filters]
      })
    )
    ...
    self._agent_service = AgentService(AgentDataProvider(addrs_fn=self._gen_addrs_map), conf=config)
    ...
    for model_name in service_map:
      for sub_graph, service_infos in service_map[model_name].items():
        addrs_map[f"{model_name}:{sub_graph}"] = [
          service_info.grpc if self._tfs_wrapper.is_grpc_remote_op else service_info.archon for service_info in service_infos]
    return addrs_map
  ...
  def sync_available_saved_models(self):
    ...
    self._backend.sync_available_saved_models(self._container, available_saved_models)
  
  def layout_update_callback(self, saved_models: List[Tuple[SavedModel, SavedModelDeployConfig]]) -> bool:
    ...
    for saved_model, deploy_config in saved_models:
      ...
      if not accepted:
        continue
      tfs_model_config = gen_model_config(name=str(saved_model), 
    ...
    logging.info(f"writing model server_config: {text_format.MessageToString(model_server_config)}")
  ...
  def start(self):
    ...
    self.start_bg_thread(
      partial(self._backend.report_service_info, self._container, self._service_info), interval=60)
    self.start_bg_thread(self.sync_available_saved_models, interval=30)
    self._backend.register_layout_callback(self.config.layout_path, self.layout_update_callback)
