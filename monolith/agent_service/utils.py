...
DEFAULT_CONF = "/opt/tiger/byterec_tce_ws/MonolithServingInst_1/agent.conf"
DEFAULT_PLATFORM_CONFIG_FILE = "/opt/tiger/monolith_serving/conf/platform_config_file.cfg"
flags.DEFINE_string("conf", DEFAULT_CONF, "agent conf file")
...
DefaultRoughSortModelLocalPath = '/opt/tiger/byterec_data/monolith_rough_sort_model_path'
DefaultRoughSortModelP2PPath = '/opt/tiger/byterec_data/monolith_model'
...
class DeployType(TFSServerType):
  ...
  def __init__(self, dtype: str):
    assert dtype.lower() in {self.ENTRY, self.PS, self.MIXED, self.UNIFIED}
...
class AgentConfig(TfServingConfig):
  ...
  attributes:
    ...
    :param deploy_type: Server type, can be ps/entry/mixed.
    ...
    :param max_waiting_sec: The waiting second for PS to load, default 600
    ...
    :param rough_sort_model_loaded_server: load rough sort model on which server: ps or entry
  ...
  def num_tce_shard(self) -> int:
    return int(os.environ.get('byterec_host_shard_n', 1))
  ...
  def get_cmd_and_port(self,
    ...
    if self.deploy_type == DeployType.MIXED and server_type == TFSServerType.PS:
      psm = psm + '_ps'
      prefix = psm
      log_conf = '../conf/log4j_ps.properties'
  ...
  def _gen_model_server_config(
    ...
    if server_type == TFSServerType.PS:
      ...
      model_config = model_config_list.add()
      model_config.CopyFrom(
          gen_model_config(name=server_type,
                           base_path=os.path.join(self.base_path, server_type),
                           version_policy=version_policy,
                           version_data=version_data))
      if self.rough_sort_model_name and self.rough_sort_model_loaded_server == RoughSortModelLoadedServer.ENTRY:
