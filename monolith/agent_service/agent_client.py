...
flags.DEFINE_enum("args", "addr", ["addr", "portal", "pub", "res", "lock", "elect", "info"], "args: addr, portal, pub, res, lock, elect")
flags.DEFINE_enum("server_type", "ps", ["ps", "entry"], "server_type, ps or entry")
...
flags.DEFINE_integer('num_shard', -1, 'number of shard will use of current model')
...
def main(_):
  ...
  if FLAGS.cmd_type == 'hb':
    ...
    for k, v in addresses.items():
      ...
      print("{k}  -> ({length}) \n\t{addrs}".format(k=k, length=len(addrs),
                                                    addrs="\n\t".join(addrs)))
  elif FLAGS.cmd_type == 'gr':
    assert model_name is not None
    request = GetReplicasRequest(server_type=server_type, task=FLAGS.task, 
                                 model_name=model_name)
    print(ServerType.Name(server_type), FLAGS.task, " => ",
          stub.GetReplicas(request).address_list.address)
  elif (FLAGS.cmd_type == 'get' and FLAGS.args == 'addr') or FLAGS.cmd_type == 'addr':
    assert model_name is not None
      ...
      for server in servers:
        ...
        if replicas:
          for replica in replicas:
            ...
            replica_id = replica
            for_print.append(f"{path_prefix}/{server}/{replica_id}\tarchon_address: {data.archon_address}\t"
    ...
    zk = MonolithKazooClient(hosts=agent_conf.zk_servers)
    zk.start()
    mm = ModelMeta(model_name=model_name, model_dir=FLAGS.model_dir,
                   ckpt=FLAGS.ckpt, num_shard=FLAGS.num_shard)
