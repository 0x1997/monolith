...
class ServingUtilsTest(unittest.TestCase):
  ...
  def test_gen_model_spec(self):
    name, version, signature_name = 'iptv', 1, 'predict'
  ...
  def test_gen_model_config(self):
    name, base_path, num_versions = 'iptv', '/tmp/iptv/saved_model', 2
    version_labels = {'v0': 0, 'v1': 1}
    model_config = utils.gen_model_config(name, base_path, version_data=num_versions,
                                          version_labels=version_labels)
    self.assertEqual(model_config.name, name)
    self.assertEqual(model_config.base_path, base_path)
    self.assertEqual(model_config.model_version_policy.latest.num_versions, num_versions)
  
  def test_gen_status_proto(self):
    status_proto = utils.gen_status_proto(utils.ErrorCode.CANCELLED, error_message='CANCELLED')
  ...
  def test_gen_model_version_status(self):
    ...
    model_version_status = utils.gen_model_version_status(version, state,
  ...
  def test_gen_from_file(self):
    conf = utils.AgentConfig.from_file(fname='monolith/agent_service/agent.conf')
  ...
  def test_instance_wrapper_from_json(self):    
    iw = utils.InstanceFormater.from_json('monolith/agent_service/test_data/inst.json')
  ...
  def test_instance_wrapper_from_pbtext(self):    
    iw = utils.InstanceFormater.from_pb_text('monolith/agent_service/test_data/inst.pbtext')
    tensor_proto = iw.to_tensor_proto(5)
    self.assertEqual(tensor_proto.dtype, 7)
    self.assertEqual(tensor_proto.tensor_shape.dim[0].size, 5)

  def test_instance_wrapper_from_dump(self):    
    iw = utils.InstanceFormater.from_dump('monolith/agent_service/test_data/inst.dump')
    tensor_proto = iw.to_tensor_proto(5)
    self.assertEqual(tensor_proto.dtype, 7)
    self.assertEqual(tensor_proto.tensor_shape.dim[0].size, 5)
  ...
  def test_get_cmd_and_port(self):
    ...
    cmd, port = conf.get_cmd_and_port(binary='tensorflow_model_server', server_type='ps')
    self.assertTrue('model_config_file_poll_wait_seconds' in cmd)

  def test_zk_path_full(self):
    zk_pzth = utils.ZKPath('/bzid/service/base_name/idc:cluster/server_type:0/1') 
  ...
  def test_zk_path_old_partial2(self):
    zk_pzth = utils.ZKPath('/1_20001223_44ce735e-d05c-11ec-ba29-00163e356637/service/20001223_zm_test_realtime_training_1328_v4_r982567_0/ps:1')
    self.assertEqual(zk_pzth.bzid, '1_20001223_44ce735e-d05c-11ec-ba29-00163e356637')
    self.assertEqual(zk_pzth.base_name, '20001223_zm_test_realtime_training_1328_v4_r982567_0')
    self.assertEqual(zk_pzth.idc, None)
    self.assertEqual(zk_pzth.cluster, None)
