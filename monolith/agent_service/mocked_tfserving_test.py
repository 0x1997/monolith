...
MODEL_NAME = 'iptv_test'
BASE_PATH = '/tmp/iptv/monolith'
...
class MockedTFSTest(unittest.TestCase):
  ...
  def test_get_model_metadata(self):
    ...
    request.model_spec.CopyFrom(utils.gen_model_spec(MODEL_NAME, 2, signature_name='predict'))
    ...
    self.assertTrue(isinstance(stub.GetModelMetadata(request), GetModelMetadataResponse))
  
  def test_get_model_status(self):
    stub = ModelServiceStub(grpc.insecure_channel(Address))
    request = GetModelStatusRequest()
    request.model_spec.CopyFrom(utils.gen_model_spec(MODEL_NAME, 1, signature_name='predict'))
    self.assertTrue(isinstance(stub.GetModelStatus(request), GetModelStatusResponse))
  ...
  def test_handle_reload_config_request(self):
    ...
    model_config_list.extend([
      utils.gen_model_config(name='iptv_ctr', base_path='/tmp/iptv/ctr/saved_model', version_data=2),
      utils.gen_model_config(name='iptv_cvr', base_path='/tmp/iptv/cvr/saved_model', version_data=1),
    ])
    self.assertTrue(isinstance(stub.HandleReloadConfigRequest(request), ReloadConfigResponse))
