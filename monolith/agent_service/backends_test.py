...
class ZKBackendTest(unittest.TestCase):
  ...
  def setUpClass(cls) -> None:
    ...
    cls.service_info = backends.ContainerServiceInfo(
      grpc="localhost:8888",
      http="localhost:8889",
    ...
    cls.backend = backends.ZKBackend(
  ...
  def test_layout_callback(self):
    def base_path(sub_graph):
      return "/tmp/yijie.zhu/test_ffm_model/exported_models/" + sub_graph
    ...
    self.backend.remove_from_layout(self.layout_path, backends.SavedModel('test_ffm_model', 'entry'))
  ...
  def test_sync_available_models(self):
    ...
    self.assertTrue(self.zk.exists(f"/gip/binding/test_ffm_model/entry:{self.container}"))
    self.assertTrue(self.zk.exists(f"/gip/binding/test_ffm_model/ps_0:{self.container}"))
    self.assertTrue(self.zk.exists(f"/gip/binding/test_ffm_model/ps_1:{self.container}"))
