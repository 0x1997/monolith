...
class AgentControllerTest(unittest.TestCase):
  ...
  def setUpClass(cls) -> None:
    cls.bzid = 'gip'
    cls.bd = backends.ZKBackend(
      cls.bzid, zk_servers='127.0.0.1:9999')
  ...
  def test_decl_saved_models(self):
    ...
    self.assertEqual(
      set(saved_models),
      {
        saved_model(sub_graph) for sub_graph in ['ps_0', 'ps_1', 'ps_2', 'ps_3', 'ps_4', 'entry']
      }
    )
  ...
  def test_pub(self):
    ...
    agent_controller.declare_saved_model(
      ...
      'test_ffm_model',
      overwrite=True)
    agent_controller.map_model_to_layout(self.bd, "test_ffm_model:entry", "/gip/layouts/test_layout1", action="pub")
    self.assertEqual(self.bd.bzid_info()['layout_info']['test_layout1'], ['test_ffm_model:entry'])
    agent_controller.map_model_to_layout(self.bd, "test_ffm_model:ps_*", "/gip/layouts/test_layout1", action="pub")
    self.assertEqual(self.bd.bzid_info()['layout_info']['test_layout1'], 
      ['test_ffm_model:entry',
       ...
       'test_ffm_model:ps_3',
       'test_ffm_model:ps_4'])
    agent_controller.map_model_to_layout(self.bd, "test_ffm_model:ps_*", "/gip/layouts/test_layout1", action="unpub")
    self.assertEqual(self.bd.bzid_info()['layout_info']['test_layout1'], ['test_ffm_model:entry'])
    agent_controller.map_model_to_layout(self.bd, "test_ffm_model:entry", "/gip/layouts/test_layout1", action="unpub")
    self.assertEqual(self.bd.bzid_info()['layout_info']['test_layout1'], [])
