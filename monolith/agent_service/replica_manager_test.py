...
class ReplicaMgrTest(unittest.TestCase):
  ...
  def setUpClass(cls) -> None:
    os.environ['byterec_host_shard_n'] = '5'
    os.environ['SHARD_ID'] = '1'
    os.environ['REPLICA_ID'] = '2'
    os.environ['TCE_INTERNAL_IDC'] = 'lf'
    os.environ['TCE_LOGICAL_CLUSTER'] = 'default'
    cls.agent_conf = utils.AgentConfig(
      bzid='iptv', base_name=MODEL_NAME, deploy_type='mixed', base_path=BASE_PATH, num_ps=20, num_shard=5, dc_aware=True)
    
    entry_cmd = cls.agent_conf.get_cmd('tensorflow_serving', server_type=utils.TFSServerType.ENTRY)
    start = entry_cmd.index('model_config_file') + len('model_config_file') + 1
    end = entry_cmd.find(' ', start)
    cls.tfs_entry = FakeTFServing(model_config_file=entry_cmd[start:end], num_versions=1, port=cls.agent_conf.tfs_entry_port)
    
    ps_cmd = cls.agent_conf.get_cmd('tensorflow_serving', server_type=utils.TFSServerType.PS)
    start = ps_cmd.index('model_config_file') + len('model_config_file') + 1
    end = ps_cmd.find(' ', start)
    cls.tfs_ps = FakeTFServing(model_config_file=ps_cmd[start:end], num_versions=1, port=cls.agent_conf.tfs_ps_port)
      ...
      for shard_id in range(self.agent_conf.num_shard):
        ...
        for task_id in range(self.agent_conf.num_ps):
          if task_id % self.agent_conf.num_shard == shard_id:
            meta = ReplicaMeta(address=f'192.168.1.{idx}:{utils.find_free_port()}', stat=ModelState.AVAILABLE)
  ...
  def test_get_replicas(self):
    ...
    try:
      ...
      self.assertTrue(task_01[0] is None and task_01[1] is None and task_01[2] is not None)
      self.assertTrue(len(mgr.get_replicas(ServerType.ENTRY, task=0)) == NUM_REPLICAS)
    finally:
      mgr.stop()
      zk.stop()
