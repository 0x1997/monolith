...
class AgentServiceImpl(AgentServiceServicer):
  ...
  def GetReplicas(self, request: GetReplicasRequest, context) -> GetReplicasResponse:
    ...
    if self.conf is None or self.conf.agent_version == 1:
      ...
      address = self._watcher.get_replicas(request.server_type, request.task, idc, cluster)
      response.address_list.address.extend(address)
    elif self.conf.agent_version == 2:
      rms: List[ReplicaMeta] = self._zk.get_task_replicas(request.model_name, request.server_type, 
                                                          request.task)
  ...
  def HeartBeat(self, request: HeartBeatRequest, context) -> HeartBeatResponse:
    ...
    if self.conf is None or self.conf.agent_version == 1:
      ...
      rm_dict: Dict[str, List[ReplicaMeta]] = self._zk.get_all_replicas(request.server_type)
  ...
  def GetResource(self, request: GetResourceRequest, context) -> GetResourceResponse:
    if self.conf is None or self.conf.agent_version == 1:
...
class AgentService:
  ...
  def _(self, data_provider: AgentDataProvider, conf: AgentConfig, max_workers: int = 10):
    self._server = grpc.server(futures.ThreadPoolExecutor(max_workers=max_workers))
    add_AgentServiceServicer_to_server(AgentServiceImpl(data_provider, conf), self._server)
    self._server.add_insecure_port(f'[::]:{conf.agent_port or 0}')
