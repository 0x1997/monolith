...
version = '1634631496'
path = '/data01/home/wangcaihua/monolith/monolith/agent_service/test_data/ckpt/exported_models/{}/{}'
...
class TFSMonitorTest(unittest.TestCase):
  ...
  def setUpClass(cls) -> None:
    ...
    cls.agent_conf = utils.AgentConfig(bzid='iptv', deploy_type='mixed')

    cls.tfs_entry = FakeTFServing(num_versions=2, port=cls.agent_conf.tfs_entry_port,
                                  model_config_file=ModelServerConfig())
    cls.tfs_ps = FakeTFServing(num_versions=2, port=cls.agent_conf.tfs_ps_port,
                               model_config_file=ModelServerConfig())
  ...
  def setUp(self):
    ...
    pm = PublishMeta(
        shard_id=self.agent_conf.shard_id,
  ...
  def tearDown(self):
    ...
    if self.data['execute'] == 'reload_config':
      for tfs_model_name, (bvp, bstate) in before_status.items():
        ...
        self.assertTrue(bstate.version == -1 and bstate.status.error_code == 5)  # NOT_FOUND
        if astate.version == -1:
          pass
  ...
  def test_reload_config(self):
    ...
    for i in range(10):
      ...
      sub_models = {f'ps_{i}': path.format(f'ps_{i}', version) for i in range(num_ps) if i % 3 == 0}
    ...
    model_configs: Dict[str, ModelServerConfig] = self.monitor.gen_model_config(pms)
    for service_type, model_config in model_configs.items():
      if len(model_config.model_config_list.config) > 0:
        status = self.monitor.handle_reload_config_request(service_type, model_config)
