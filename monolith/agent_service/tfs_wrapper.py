...
TFS_BINARY = os.environ.get('MONOLITH_TFS_BINARY', '/opt/tiger/monolith_serving/bin/tensorflow_model_server')
...
class TFSWrapper(object):
  ...
  def __init__(self,
    ...
    cp = subprocess.run(f"strings {TFS_BINARY} | grep PredictionServiceGrpc", shell=True)
  ...
  def _prepare_cmd(self):
    ...
    if not self._is_grpc_remote_op:
      flags.append(f'--archon_entry_to_ps_rpc_timeout={self._binary_config.fetch_ps_timeout_ms}')
  ...
  def start(self):
    ...
    logging.info(f"starting {'grpc' if self._is_grpc_remote_op else 'archon'} tfs in {os.getcwd()} using command {tfs_cmd}")
  ...
  def list_saved_models(self):
    model_server_config = text_format.Parse(self.model_config_text(), model_server_config_pb2.ModelServerConfig())
  ...
  def list_saved_models_status(self):
    ...
    for saved_model in saved_models:
      ...
      try:
          model_version_status = self._stub.GetModelStatus(request).model_version_status
          if model_version_status is None or len(model_version_status) == 0:
            ...
            available_version_status = [mvs for mvs in model_version_status if mvs.state == State.AVAILABLE]
