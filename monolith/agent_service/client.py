...
FLAGS = flags.FLAGS
flags.DEFINE_enum("cmd_type", "hb", ["hb", "gr", "addr", "get", "clean", "load", "unload", "meta", "status"], "cmd_type: hb, gr, addr, res, status")
...
flags.DEFINE_enum("input_type", 'dump', ["json", "pbtext", "dump", "binary", "instance", "example_batch", "example_batch_to_instance"], "inputs type for prediction")
...
class ServingClient(object):
  ...
  def load(self, model_name: str,
           ...
           ckpt: str = None,
           num_shard: int = -1):
    mm = ModelMeta(model_name=model_name, model_dir=model_dir,
                   ckpt=ckpt, num_shard=num_shard)
  ...
  def get_status(self, model_name: str) -> LoadSate:
    ...
    for node in self.kazoo.get_children(f'/{self.bzid}/service/{model_name}'):
      for replica in self.kazoo.get_children(f'/{self.bzid}/service/{model_name}/{node}'):
