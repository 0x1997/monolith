...
class ModelMgr:
  ...
  def load(self, model_config_list):
    for config in model_config_list:
      ...
      model_conf = ModelConf(config.name, config.base_path,
                             version_policy=version_policy, version_data=version_data)
      self._models[config.name] = ModelMeta(model_conf, versions)

      logging.info('start load a new model {}'.format(config.name))
      for v in versions:
        self._queue.put(Event(model_name=config.name, version=v.version, state=ModelState.START))
  ...
  def remove(self, model_name_list):
    for model_name in model_name_list:
      ...
      for version in model.versions:
        self._queue.put(Event(model_name, version.version, ModelState.UNLOADING))
  ...
  def get_status(self, model_spec):
    ...
    with self._lock:
      ...
      if len(model_version_status) == 0:
        mvs = gen_model_version_status(-1, error_code=ErrorCode.NOT_FOUND,
    ...
    while not self._has_stopped:
      ...
      if end_time - start_time > 30:
        ...
        if model_conf.version_policy != 'specific':
          ...
          logging.info('start load a new version of model {}'.format(model_name))
  ...
  def _event_handler(self, event: Event):
    with self._lock:
        ...
        for version in model.versions:
          if version.version == event.version:
            if version.state == ModelState.UNKNOWN:
              ...
              self._queue.put(Event(event.model_name, event.version, ModelState.LOADING))
              log_flag = True
            break
        ...
        for version in model.versions:
          if version.version == event.version:
            if version.state == ModelState.START:
              version.state = event.state
              self._queue.put(Event(event.model_name, event.version, ModelState.AVAILABLE))
              log_flag = True
            break
            ...
            if version.state == ModelState.LOADING:
              ...
              if model.conf.version_policy == 'latest':
                if len(model.versions) > model.conf.version_data:
                  self._queue.put(Event(event.model_name, model.versions[0].version,
        ...
        for version in model.versions:
          if version.version == event.version:
            ...
            if version.state not in {ModelState.UNLOADING, ModelState.END}:
              version.state = event.state
              self._queue.put(Event(event.model_name, event.version, ModelState.END))
              log_flag = True
            break
        ...
        for i, version in enumerate(model.versions):
          if version.version == event.version:
            if version.state == ModelState.UNLOADING:
              version.state = event.state
              logging.info(f'{event.model_name}-{event.version}: state is {ModelState.Name(event.state)}')
            index = i
            break
        
        if index >= 0:
          logging.info(f'{event.model_name}-{model.versions[index].version} is removed!')
      ...
      if log_flag:
        logging.info(f'{event.model_name}-{event.version}: state is {event.state}')
...
class ModelServiceImpl(ModelServiceServicer):
  ...
  def HandleReloadConfigRequest(self, request: ReloadConfigRequest, context):
    ...
    self._model_mgr.load([config for config in model_config_list
                          if config.name in to_load])
...
class FakeTFServing:
  def __init__(self, model_name: str = None, base_path: str = None, num_versions: int = 1,
               port: int = 8500, max_workers: int = 10, model_config_file = None):
    if model_config_file is None:
      self._model_mgr = ModelMgr([gen_model_config(model_name, base_path, version_data=num_versions)])
      ...
      assert isinstance(model_config_file, model_server_config_pb2.ModelServerConfig)
      self._model_mgr = ModelMgr(model_config_file.model_config_list.config)
    
    self._server = grpc.server(futures.ThreadPoolExecutor(max_workers=max_workers))
    add_ModelServiceServicer_to_server(ModelServiceImpl(self._model_mgr), self._server)
    add_PredictionServiceServicer_to_server(PredictionServiceImpl(self._model_mgr), self._server)
...
if __name__ == '__main__':
  tfs = FakeTFServing('iptv_test', '/tmp/iptv/monolith', num_versions=1)
  tfs.start()
