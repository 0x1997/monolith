...
from monolith.agent_service.agent_base import AgentBase, ServingLog, TFS_HOME, get_cmd_and_port
...
class ProcessNode(object):
  def __init__(self, 
    ...
    if proc_type == ProcessType.PS:
      self._cmd, self._port = get_cmd_and_port(config, conf_path, TFSServerType.PS)
    elif proc_type == ProcessType.ENTRY:
      self._cmd, self._port = get_cmd_and_port(config, conf_path, TFSServerType.ENTRY)
    else:
      self._cmd, self._port = get_cmd_and_port(config, conf_path)
  ...
  def run(self):
    ...
    if self.proc_type == ProcessType.ENTRY:
      ...
      while not self._replica_mgr.is_ps_set_started() and waiting_sec < max_waiting_sec:
        time.sleep(self._config.update_model_status_interval * 2)
        waiting_sec += self._config.update_model_status_interval * 2
  ...
  def failover(self):
    ...
    if not self.is_tce_main and self.proc_type == ProcessType.PS:
    ...
    while not self._thread_stopped and not ProcessMgr._is_killed:
      ...
      for proc in procs:
        ...
        if proc.returncode is not None:
          logging.info(f"{proc.proc_type} {proc.returncode} shutdown, kill all proc...")
          #proc.failover()
          #先简化管理, 有进程挂掉的话就整体挂了
          self.kill_all()
...
class AgentV1(AgentBase):
  def __init__(self, config: AgentConfig, conf_path:str, tfs_log: str):
    ...
    self.agent_service = AgentService(self.replica_mgr.watcher, port=config.agent_port)

    pm = ProcessMgr()
    if config.deploy_type == DeployType.MIXED:
      ps_proc = ProcessNode(config, self.replica_mgr, proc_type=ProcessType.PS, 
                            conf_path=conf_path, tfs_log=tfs_log)
      pm.add_subproc(ps_proc)

      entry_proc = ProcessNode(config, self.replica_mgr, proc_type=ProcessType.ENTRY, 
                               is_tce_main=True, conf_path=conf_path, tfs_log=tfs_log)
      pm.add_subproc(entry_proc)
    elif config.deploy_type == DeployType.ENTRY:
      proxy_proc = ProcessNode(config, self.replica_mgr, proc_type=ProcessType.ENTRY, 
                               is_tce_main=True, conf_path=conf_path, tfs_log=tfs_log)
