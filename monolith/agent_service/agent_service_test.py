...
class AgentServiceTest(unittest.TestCase):
  ...
  def setUpClass(cls) -> None:
    ...
    cls.zk.start()
    cls.agent_conf: utils.AgentConfig = utils.AgentConfig(
      bzid='iptv', base_name=MODEL_NAME, deploy_type='ps', base_path=BASE_PATH, num_ps=20, dc_aware=True)
  ...
  def test_get_replicas(self):
    resp = self.client.get_replicas(server_type=ServerType.PS, task=NUM_PS_REPLICAS-1)
