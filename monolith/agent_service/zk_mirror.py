...
class ZKMirror(object):
  ...
  def __init__(self,
    ...
    # /{bzid}/{model_name}/deploy_type:task_id/replica -> ReplicaMeta
  ...
  def create(self, path, value=b"", acl=None, ephemeral=False,
             sequence=False, makepath=True, include_data=False):
    with self._zk_lock:
      try:
        self._zk.create(path, value, acl=acl, ephemeral=ephemeral, 
  ...
  def resources(self) -> List[ResourceSpec]:
    # /{bzid}/resource/{shard_id}:{replica_id}  -> ResourceSpec
    with self._data_lock:
      resource_paths = [os.path.join(self.resource_path, child)
  ...
  def num_tce_replica(self) -> Optional[int]:
    ...
    while not all(cnt == self.tce_shard_id for cnt in replicas.values()):
      time.sleep(5)
      # log every minute
      logging.log_every_n(level=logging.INFO, msg='cluster autoscaler or broken node, keep waiting', n=12)
      replicas = get_replica_cnt()
  ...
  def expected_loading(self) -> Dict[ModelName, PublishMeta]:
    ...
    with self._data_lock:
      ...
      for node in nodes:
        ...
        elif len(shortest_sub_model_pm[model_name].sub_models) > len(pm.sub_models):
          shortest_sub_model_pm[model_name] = pm
      ...
      for pm in select:
        path = os.path.join(self.publish_base_path, 
        ...
        if pm.shard_id == self.tce_shard_id and pm.replica_id == self.tce_replica_id:
          ...
          # only ps/mixed service type will hit this branch
          ...
          # all entry/ps/mixed service type can hit this branch
          # and ps submodels were filtered
          if model_name not in expected:
            pm.shard_id = self.tce_shard_id
            pm.replica_id = self.tce_replica_id
            pm.sub_models = {sub_model_name: vp for sub_model_name, vp in pm.sub_models.items()
  ...
  def get_published_path(self, model_name: str) -> List[str]:
    with self._data_lock:
      paths = []
      for path in self._data:
        if path.startswith(self.publish_base_path) and path.endswith(model_name):
          paths.append(path)
      return paths
  ...
  def watch_portal(self):
    ...
    models_in_portal = set(self._zk.get_children(path=self.portal_base_path) or [])
    models_in_publish = {item.split(':')[-1]  # {shard_id}:{model_name} -> model_name
    ...
    def create_data_watch(data_path: str):
      logging.info(f"add data watch for model {os.path.basename(data_path)} in portal")

      def data_watch(data: bytes, state: ZnodeStat, event: WatchedEvent):
        # info = ModelMeta.deserialize(data)
        with self._data_lock:
          if event is None or event.type in {EventType.CREATED, EventType.DELETED}:
            ...
            if data is not None and len(data) > 0:
              ...
              mm = ModelMeta(model_name=os.path.basename(data_path), action=action)
            self.queue.put(Event(data_path, mm.serialize(), EType.PORTAL))
          else:
            assert event.type in {EventType.CHILD, EventType.CHANGED, EventType.NONE}
      return data_watch
    ...
    self._zk.ChildrenWatch(path=self.portal_base_path,
                           func=children_watch)
  ...
  def watch_publish(self):
    ...
    self._zk.ChildrenWatch(path=self.publish_base_path,
                           func=children_watch)
  ...
  def watch_resource(self):
    ...
    self._zk.ChildrenWatch(path=self.resource_path,
                           func=children_watch)
  ...
  def watch_service(self):
    ...
    self._zk.ChildrenWatch(path=self.service_base_path,
                           func=model_watch)
  ...
  def election(self, leader, sched, identifier: str = None):
    ...
    def target():
      try:
        election: Election = self._zk.Election(self._zk_election_path, identifier)
