...
class ChildrenWatch:
  def __init__(self,
               client,
               path: str,
               func: Union[Callable[[List[str]], None], Callable[[List[str], WatchedEvent], None]],
               send_event=False):
    self.path = path
...
class DataWatch:
  def __init__(self, client, path: str,
               func: Union[Callable[[bytes, ZnodeStat, WatchedEvent], None], Callable[[bytes, ZnodeStat], None]]):
    self.path = path
    self._func = func
    catalog = client._catalog
...
class Node:
  def __init__(self, path: str, value: bytes = b'', ephemeral: bool = False,
               data_watch: DataWatch = None, children_watch: ChildrenWatch = None):
    self.path: str = path
    ...
    event = WatchedEvent(type=EventType.CREATED, state=KeeperState.CONNECTED, path=self.path)
  ...
  def set(self, value: bytes):
    ...
    if self._data_watch is not None:
      event = WatchedEvent(type=EventType.CHANGED, state=KeeperState.CONNECTED, path=self.path)
      self._data_watch(self.value, self.state, event)
  ...
  def create_child(self, path: str, value: bytes = b'', ephemeral: bool = False,
                   data_watch=None, children_watch=None):
    ...
    if self._children_watch is not None:
      event = WatchedEvent(type=EventType.CHILD, state=KeeperState.CONNECTED, path=self.path)
  ...
  def __del__(self):
    event = WatchedEvent(type=EventType.DELETED, state=KeeperState.CONNECTED, path=self.path)
    if self._data_watch is not None:
      self._data_watch(self.value, self.state, event)
...
class Catalog:
  ...
  def create(self, path: str, value: bytes = b'',
             ephemeral: bool = False, makepath: bool = False, sequence: bool = False):
    if sequence:
    ...
    if pnode.has_child(path):
      ...
      return pnode.create_child(path, value, ephemeral, data_watch, children_watch)
...
class FakeKazooClient:
  ...
  def create(self, path: str, value: bytes = b'', acl=None,
             ephemeral: bool = False, makepath: bool = False, include_data: bool = False, sequence: bool = False):
