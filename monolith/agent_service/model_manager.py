...
import sys
sys.path.append('/opt/tiger/pyutil')
import os
import threading
import time
import shutil
...
class ModelManager(object):
    ...
    def init_metrics(self):
        ...
        conf = {}
        default_psm = 'data.monolith_serving.' + self._model_name
        conf['psm'] = os.environ.get('TCE_PSM', default_psm)
        conf['metrics_namespace_prefix'] = 'data.monolith_serving.online'
        self._metrics.init(conf)
        self._metrics.define_counter('loop_once_failed')
        self._metrics.define_store('version.delay')
        self._metrics.define_store('update.delay')
    ...
    def _start(self):
        ...
        self._loop_thread = threading.Thread(target=self.run, name="thread-model_manager")
    ...
    def check_model_update_time(self):
        ...
        if self._model_name not in self._latest_models:
            logging.error('model %s not in _latest_models: %s', self._model_name, str(self._latest_models))
            self._metrics.emit_counter('loop_once_failed', 1,  tagkv={'model': self._model_name})
            return
        version, update_time = self._latest_models[self._model_name]
        cur_time = int(time.time())
        self._metrics.emit_store('version.delay', cur_time - int(version), tagkv={'model': self._model_name})
        self._metrics.emit_store('update.delay', cur_time - update_time, tagkv={'model': self._model_name})
    ...
    def loop_once(self):
        ...
        for model_name in source_data:
            ...
            ret, file_list = self.copy_model(model_name, new_version, source_data[model_name][1])
            if ret:
    ...
    def copy_model(self, model_name, version, model_data):
         ...
         for sub_model_name, sub_model_data in model_data:
             ...
             try:
                 ...
                 logging.error('copy model %s -> %s faild: %s', src_file, temp_dst_file, str(err))
                 self.delete(temp_dst_file)
                 break

         if ready_num != sub_model_num:
             logging.error(f'copy model faild, ready_num={ready_num}, expect_num={sub_model_num}')
...
if __name__ == '__main__':
    ...
    source_path = '/data00/home/gaoxiang.bupt/tmp/local_model'
    receive_path = '/data00/home/gaoxiang.bupt/tmp/load_model'
    manager = ModelManager('hupu_deep_v4', source_path, receive_path, True)
    #ret = manager.wait_for_download()
    #source_data = manager.get_source_data()
    #logging.info(f'ret is {source_data}')
    manager.start()
    time.sleep(20)
    manager.stop()
