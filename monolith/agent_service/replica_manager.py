...
from monolith.agent_service.resource_utils import _export_saver_listener_state_parser, _ExportSaverListenerStateFile
...
class ReplicaUpdater(object):
  ...
  def init_metrics(self):
    ...
    self._metrics_cli = metrics.Client(
  ...
  def _do_register(self, replica_path: str, grpc_port: int, archon_port: int):
    ...
    entry_replica_meta = ReplicaMeta(
    ...
    self.meta[replica_path] = entry_replica_meta
    replica_meta_bytes = bytes(entry_replica_meta.to_json(), encoding='utf-8')
    try:
      ...
      if self._conf.replica_id == -1:
        ...
        self.meta[real_path] = entry_replica_meta
  ...
  def _do_update(self, name: str):
    if name.startswith(TFSServerType.ENTRY):
      ...
      replica_path = f'{self.path_prefix}/{TFSServerType.PS}:{name.split("_")[1]}/{self._conf.replica_id}'
  ...
  def _get_latest_version_in_fs(self, name):
    ...
    state_file_path = os.path.join(exported_models_dir,
                                   _ExportSaverListenerStateFile)
    stat_dict = _export_saver_listener_state_parser(state_file_path)
    for name, values in stat_dict.items():
      if name == exported_models_dir:
        latest_version = int(values[-1][0])
...
class ReplicaManager:
  ...
  def is_ps_set_started(self):
    for i in range(self._conf.num_ps):
      replicas = self._watcher.get_replicas(ServerType.PS, i)
