...
class ZKMirrorTest(unittest.TestCase):
  ...
  def test_crud(self):
    # ensure_path
    self.zk.ensure_path(path='/iptv/crud')
    # exists
    self.assertTrue(self.zk.exists(path='/iptv/crud'))
    # create
    self.zk.create(path='/iptv/crud/data', value=b'test', makepath=True)
    # get/set
    value, _ = self.zk._zk.get(path='/iptv/crud/data')
    self.assertEqual(value, b'test')
    self.zk.set(path='/iptv/crud/data', value=b'new_test')
    value, _ = self.zk._zk.get(path='/iptv/crud/data')
    self.assertEqual(value, b'new_test')
    # delete
    self.zk.delete(path='/iptv/crud', recursive=False)
    self.assertFalse(self.zk.exists(path='/iptv/crud'))
  ...
  def test_zk_mirror(self):
    ...
    for i in range(mm.num_shard):
      sub_models: Dict[SubModelName, VersionPath] = {
        f'ps_{k}': f'{mm.model_dir}/ps_{k}/{version}' for k in range(num_ps) if k % mm.num_shard == i
      }
    ...
    entry_replica = ReplicaMeta(address=f'{local_ip}:8080', model_name='iptv_ctr', server_type='entry', 
                                task=0, replica=2, stat=30)
    ps0_replica = ReplicaMeta(address=f'{local_ip}:8080', model_name='iptv_ctr', server_type='ps', 
                              task=0, replica=2, stat=30)
    ps5_replica = ReplicaMeta(address=f'{local_ip}:8080', model_name='iptv_ctr', server_type='ps', 
                              task=5, replica=2, stat=30)
    
    all_replicas = self.zk.get_all_replicas(server_type='ps')
    self.assertEqual(all_replicas['iptv_ctr:ps:0'][0], ps0_replica)
    self.assertEqual(all_replicas['iptv_ctr:ps:5'][0], ps5_replica)

    model_replicas = self.zk.get_model_replicas(model_name=MODEL_NAME, server_type='entry')
    self.assertEqual(model_replicas['iptv_ctr:entry:0'][0], entry_replica)

    task_replicas = self.zk.get_task_replicas(model_name=MODEL_NAME, server_type='ps', task=0)
    self.assertEqual(task_replicas[0], ps0_replica)

    self.assertEqual(ps5_replica, self.zk.get_replica(model_name=MODEL_NAME, server_type='ps', task=5, replica=2))

    local_replica_paths = {'/iptv/service/iptv_ctr/ps:0/2', 
                           '/iptv/service/iptv_ctr/entry:0/2', 
                           '/iptv/service/iptv_ctr/ps:5/2'}
