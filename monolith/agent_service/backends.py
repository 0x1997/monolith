...
class AgentBackend(abc.ABC):
  ...
  @abc.abstractmethod
  def register_layout_callback(self, layout_path: str, 
                               callback: Callable[[List[Tuple[SavedModel, SavedModelDeployConfig]]], None]) -> None:
    """
  ...
  @abc.abstractmethod
  def report_service_info(self, container: Container, service_info: ContainerServiceInfo) -> None:
    pass
...
class CtrlBackend(abc.ABC):
  ...
  @abc.abstractmethod
  def decl_saved_model(self, saved_model: SavedModel, deploy_config: SavedModelDeployConfig):
    pass
...
class ZKBackend(AgentBackend, CtrlBackend, SyncBackend):
  ...
  def sync_available_saved_models(self, container: Container, saved_models: Set[SavedModel]) -> None:
    """
    ...
    with self._lock:
      ...
      logging.info(f"available saved models updating, add: {add_saved_models}, remove: {remove_saved_models}")
  ...
  def register_layout_callback(self, layout_path: str, callback: Callable[[List[Tuple[SavedModel, SavedModelDeployConfig]]], bool]):
    """
    Invoke {callback} on layout updates(adding or removing saved_models)
    """
      ...
      with self._lock:
        ...
        for child in children:
          ...
          if data is None:
            ...
            continue
          saved_models.append((saved_model, SavedModelDeployConfig.deserialize(data)))
        ...
        for model_name in model_names:
          ...
          self._children_watch(binding_watch_path, partial(self._bind_callback, model_name))
  ...
  def report_service_info(self, container: Container, service_info: ContainerServiceInfo) -> None:
    ...
    self.create_znode(service_info_path, service_info.serialize(), ephemeral=True, makepath=True)
  ...
  def _children_watch(self, path, callback):
    with self._lock:
      if path in self._children_watcher_map and not self._children_watcher_map[path]._stopped:
        logging.info(f"active watcher exists on path {path}")
      else:
        self._zk.ensure_path(path) # make sure the path exists otherwise the watcher may not be effective
        self._children_watcher_map[path] = self._zk.ChildrenWatch(path, callback)
  ...
  def decl_saved_model(self, saved_model: SavedModel, deploy_config: SavedModelDeployConfig):
    ...
    self.create_znode(saved_model_path, deploy_config.serialize(), makepath=True)
  ...
  def bzid_info(self):
    ...
    if self._zk.exists(f"/{self._bzid}/saved_models"):
      ...
      for model_name in model_names:
        sub_graphs = self._zk.get_children(f"/{self._bzid}/saved_models/{model_name}")
        model_info[model_name]['sub_graphs_total'] = len(sub_graphs)
        for sub_graph in sub_graphs:
          model_info[model_name][sub_graph]['deploy_config'] = self.get_znode(
            f"/{self._bzid}/saved_models/{model_name}/{sub_graph}").decode('utf-8')
      ...
      for model_name in model_names:
        ...
        for binding in bindings:
          ...
          if 'bindings' not in model_info[model_name][sub_graph]:
            ...
            model_info[model_name]['sub_graphs_available'] = model_info[model_name].get('sub_graphs_available', 0) + 1
          model_info[model_name][sub_graph]['bindings'].append(f"{cluster}:{container_id}")
          if 'saved_models' not in container_info[cluster][container_id]:
            container_info[cluster][container_id]['saved_models'] = []
          container_info[cluster][container_id]['saved_models'].append(f"{model_name}:{sub_graph}")
    def sorted_dict(d):
      return dict(sorted(d.items()))
    return {
      'model_info':  {model_name: sorted_dict(model_info[model_name]) for model_name in model_info},
      'container_info': {cluster: sorted_dict(container_info[cluster]) for cluster in container_info},
  ...
  def subscribe_model(self, model_name: str):
    ...
    self._service_info_map[model_name] = self._service_info_map.get(model_name, {})
    binding_watch_path = f"/{self._bzid}/binding/{model_name}"
    self._children_watch(binding_watch_path, partial(self._bind_callback, model_name))
  ...
  def get_sync_targets(self, sub_graph: str) -> Tuple[str, List[str]]:
    with self._lock:
      ...
      return f"{self._sync_model_name}:{sub_graph}", [service_info.grpc for service_info in service_infos]


  def create_znode(self, path, value, ephemeral=False, makepath=False) -> None:
    with self._lock:
      try:
        self._zk.create(path, value=value, ephemeral=ephemeral, makepath=makepath)
