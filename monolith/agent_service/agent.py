...
FLAGS = flags.FLAGS
flags.DEFINE_string('tfs_log', '/var/log/tiger/tfs.std.log', 'The tfs log file path')
...
def main(_):
  ...
  if config.agent_version == 1:
    ...
    agent = AgentV2(config, conf_path, FLAGS.tfs_log)
  ...
  model_manager = ModelManager(config.rough_sort_model_name, config.rough_sort_model_p2p_path, 
